...
- Makes **Use cloud version** persist the displayed encrypted-cloud payload and revision as the active local base before removing the pending device copy.
- Makes **Use this device** rebase the pending local copy on the displayed cloud revision before retrying synchronization.
- Treats missing or changed conflict state as an explicit error instead of silently doing nothing.
- Adds visible processing, success, browser-storage failure, and stale-conflict feedback to the comparison popup.
- Adds behavioral regression coverage that applies both choices and verifies the resulting base, pending, and conflict records.
- Finance Schema 12, Cloud Schema V3, encryption, stored finance data, calculations, and product styling remain unchanged.
...
- Uses the same high-contrast mint active state with dark-teal text in light and dark themes.
- Keeps one visible desktop navigation control by hiding the top-bar hamburger while the icon rail is available.
- Replaces the rail hamburger with distinct pin and unpin icons plus matching accessible labels.
- Renames navigation groups to Overview, Finance, Work, and Insights while preserving every destination label and route.
- Gives collapsed-rail tooltips more time to appear before hover expansion and slightly improves expanded group-label readability.
- Adds browser coverage for dark-theme contrast, one desktop toggle, group terminology, and preserved mobile drawer controls.
- Finance Schema 12, Cloud Schema V3, saved records, calculations, and mobile navigation behavior remain unchanged.

## V14.0.4 · Agenda Workflow & Responsive Navigation

- Compacts Project Agenda into a three-event preview and adds a responsive full-agenda popup with upcoming and completed sections.
- Adds Complete and Reopen controls, optional completion metadata, confirmation before moving a linked project to Completed Projects, and preserves project payment values.
- Uses labeled urgency colors across the agenda and Dashboard calendar: overdue, near, soon, later, and completed.
- Replaces ambiguous sync-conflict resolution buttons with a local-versus-cloud comparison popup and explicit Use this device, Use cloud version, Download both, and Resolve later actions.
- Adds a persistent desktop icon rail with SVG icons, active-page highlighting, tooltips, hover/focus expansion, and a saved pinned-open state while preserving the mobile drawer.
- Finance Schema 12, Cloud Schema V3, encrypted synchronization, Supabase tables, financial calculations, and existing agenda records remain unchanged.

## V14.0.3 · Project Agenda & Dashboard Calendar

- Removed the duplicate monthly calendar grid from Projects and kept one complete **Project Agenda** ordered by date and time.
- Preserved Schedule, Add, Edit, Delete, reminders, linked projects, ICS downloads, the existing agenda storage key, and every saved agenda record.
- Projects every valid agenda date onto the Dashboard monthly calendar with the event title, type, time, and location.
- Refreshes Dashboard agenda markers and selected-day events immediately after local changes and after agenda updates from another browser tab.
- Finance Schema 12, Cloud Schema V3, encrypted synchronization, financial calculations, and Supabase tables are unchanged.

## V14.0.2 · Version Display & Sync Conflict Recovery

- Uses the central app version for the browser title, top-bar build badge, Settings overview, offline cache, and release history; the V13.0.20 calendar module no longer overwrites current release metadata.
- Makes routine account-ledger recalculation pure so startup, rendering, and hourly refreshes do not generate a new settings timestamp or save unchanged finance data.
- Keeps `ledgerSettings.lastRecalculatedAt` device-local instead of synchronizing it through `settings:preferences`.
- Sanitizes existing local cloud base, pending, and conflict records and automatically clears or safely rebases conflicts that only involved the derived ledger timestamp. Genuine overlapping preference changes remain available for manual review.
- Finance Schema 12, Cloud Schema V3, encryption, account balances, ledger entries, and Supabase tables are unchanged.

## V14.0.1 · Repository Quality & Release Hardening

- Restored executable permissions for the macOS installer and audit script, with automated permission checks to prevent regressions.
- Replaced the no-op lint command with ESLint and added portable Playwright privacy/input tests to CI.
- Extracted more than 5,300 lines of inline styling from `index.html` into a versioned `app.css` asset and added growth limits for both files.
- Aligned app, cache, installer, README, changelog, package, and validation metadata on V14.0.1.
- Added npm Dependabot coverage, maintainability limits, conventional commit guidance, and a tag-driven GitHub release workflow.
- Finance Schema 12, Cloud Schema V3, encryption, profiles, ledger behavior, calculations, and stored records are unchanged.
