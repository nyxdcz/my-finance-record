(function applyTalaanReleaseLayer() {
  ...
  function ensureLiquidGlassStyles() {
    ...
    link.rel = "stylesheet";
    link.href = "./liquid-glass.css?v=2.0.1-talaan5";
...
(function loadExpenseScreenshotTools() {
  ...
  async function start() {
    ...
    toolsPromise = (async () => {
      await loadScript("./expense-screenshot-parser.js?v=2.0.1-talaan5", "expenseScreenshotParserScript");
      await loadScript("./expense-screenshot-detect.js?v=2.0.1-talaan5", "expenseScreenshotDetectScript");
      await loadScript("./expense-screenshot-ai.js?v=2.0.1-talaan5", "expenseScreenshotAiScript");
      window.FinanceExpenseScreenshot?.ensurePanel?.();
      window.FinanceExpenseScreenshotAI?.ensureAiControls?.();
      ensureCompactScreenshotUi();
      document.getElementById("expenseScreenshotLauncherButton")?.remove();
