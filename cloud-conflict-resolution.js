...
/* My Finance Records V14.0.7 · Transactional Cloud Sync conflict state transitions. */
(function financeCloudConflictResolutionBootstrap() {
  ...
  function apply({ key, choice, item, conflict, baseRecords, pending, conflicts, setConflicts, persist, clone, splitKey, nowIso, appVersion, appVersionCode }) {
    if (!item || !conflict) throw new Error("This conflict changed before your choice was applied. Reload and review the latest versions.");
    ...
    return {key,choice,revision:nextBase.revision};
  }
