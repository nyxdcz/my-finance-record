...
(function exposeFinancePwaUpdate(root) {
  ...
  function comparisonMarkup(current, previous) {
    if (!previous) {
      return '<div class="cash-flow-summary-change comparison-neutral"><span>vs previous month</span><strong>No previous month</strong></div>';
    }
    ...
    const percent = Math.abs(previous.balance.value) > .005
      ? ` · ${Math.abs((delta / previous.balance.value) * 100).toFixed(1)}%`
      : "";
  ...
  function upgradeCashFlowLayout() {
    ...
    const comparisonPanel = panels[1];
    const rows = readCashFlowRows(comparisonPanel);
    ...
    summaryPanel.innerHTML = `
      <div class="cash-flow-summary-head"><h4>Monthly summary</h4><strong>${escapeHtml(current.label)}</strong></div>
      <div class="cash-flow-summary-metrics">
        <div class="cash-flow-summary-metric"><span><i class="income" aria-hidden="true"></i>Income</span><strong class="income-value">${escapeHtml(current.income.text)}</strong></div>
        <div class="cash-flow-summary-metric"><span><i class="expense" aria-hidden="true"></i>Expenses</span><strong class="expense-value">${escapeHtml(current.expenses.text)}</strong></div>
        <div class="cash-flow-summary-metric"><span><i class="balance" aria-hidden="true"></i>Balance</span><strong class="balance-value">${escapeHtml(current.balance.text)}</strong></div>
      </div>
      ${comparisonMarkup(current, previous)}
    `;
    ...
    try {
      if (typeof HELP_CONTENT !== "undefined" && HELP_CONTENT["cash-flow"]) {
        HELP_CONTENT["cash-flow"].summary = "Shows monthly Income and Expenses as columns, Balance as a line, and a compact summary for the selected month.";
        HELP_CONTENT["cash-flow"].calculation = "Balance equals project income received minus included Monthly Expenses.";
        HELP_CONTENT["cash-flow"].note = "The monthly summary compares Balance with the previous month. Balance here is not Account Balance, Available Money, Money Remaining, or Savings.";
      }
    } catch (error) {}
  }
