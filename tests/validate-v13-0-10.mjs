...
assert(version.version==="13.0.10","version.json is not V13.0.10");
...
assert(html.includes('<title>My Finance Records · V13.0.10</title>'),"HTML title mismatch");
assert(html.includes('const APP_VERSION = "13.0.10";'),"HTML APP_VERSION mismatch");
...
assert(worker.includes('const APP_VERSION = "13.0.10";'),"service-worker version mismatch");
...
assert(packageJson.version==="13.0.10"&&packageLock.version==="13.0.10","package version mismatch");
assert(packageJson.scripts?.quality==="node tests/validate-v13-0-10.mjs","quality script mismatch");
...
assert(readme.includes("My_Finance_Records_V13_0_10_File_Inspection_and_Fixes_Installer.zip")&&readme.includes("Install_V13_0_10.command"),"README V13.0.10 installer filenames are stale");
...
assert(readme.startsWith("# My Finance Records · V13.0.10 PWA"),"README V13.0.10 heading mismatch");
...
assert(changelog.includes("## 13.0.10 · 2026-08-07"),"CHANGELOG V13.0.10 entry missing");
...
assert(version.cacheVersion.includes("v13010-account-spending-r1"),"V13.0.10 cache revision mismatch");
...
assert(html.includes('id="settingsOverviewAppStatus">Version 13.0.10<'),"Settings overview version was not updated");
...
assert(version.name==="Account Spending from Balance", "V13.0.10 version name mismatch");
...
if(failures.length){
  console.error("V13.0.10 Account Spending from Balance validation failed:\n"+failures.map(item=>`- ${item}`).join("\n"));
  process.exit(1);
}
console.log("V13.0.10 Account Spending from Balance validation passed.");
