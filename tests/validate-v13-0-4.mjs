...
assert(version.version==="13.0.4","version.json is not V13.0.4");
...
assert(html.includes('<title>My Finance Records · V13.0.4</title>'),"HTML title mismatch");
assert(html.includes('const APP_VERSION = "13.0.4";'),"HTML APP_VERSION mismatch");
...
assert(worker.includes('const APP_VERSION = "13.0.4";'),"service-worker version mismatch");
...
assert(packageJson.version==="13.0.4"&&packageLock.version==="13.0.4","package version mismatch");
assert(packageJson.scripts?.quality==="node tests/validate-v13-0-4.mjs","quality script mismatch");
...
assert(readme.startsWith("# My Finance Records · V13.0.4 PWA"),"README heading mismatch");
...
assert(changelog.includes("## 13.0.4 · 2026-08-07"),"CHANGELOG V13 entry missing");
...
const syntaxFiles=[
  "security-profiles.js","cloud-sync.js","account-ledger.js","budget-planning.js","reports-insights.js","productivity-tools.js","reminders-alerts.js","sw.js","tests/validate-v13-0-4.mjs"
];
...
assert(version.cacheVersion.includes("v13004-settings-topbar-r1"),"Settings cache revision mismatch");
...
assert(!worker.includes('const APP_VERSION = "13.0.3"'),"service worker still reports V13.0.3");
...
assert(html.includes('id="settingsOverviewAppStatus">Version 13.0.4<'),"Settings overview version was not updated");
...
if(failures.length){
  console.error("V13.0.4 Settings & Top Bar UI validation failed:\n"+failures.map(item=>`- ${item}`).join("\n"));
  process.exit(1);
}
console.log("V13.0.4 Settings & Top Bar UI validation passed.");
