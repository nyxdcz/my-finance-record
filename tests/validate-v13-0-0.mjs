...
assert(version.version==="13.0.0","version.json is not V13.0.0");
...
assert(html.includes('<title>My Finance Records · V13.0.0</title>'),"HTML title mismatch");
assert(html.includes('const APP_VERSION = "13.0.0";'),"HTML APP_VERSION mismatch");
...
assert(html.includes('{"version": "V13.0.0", "title": "Major Cloud, Encryption & Profile Architecture"'),"in-app V13 history entry missing");
assert(html.includes('{"version": "V12.21.0", "title": "Record-level Cloud Sync 2.0"'),"V12.21 history was rewritten incorrectly");
assert(worker.includes('const APP_VERSION = "13.0.0";'),"service-worker version mismatch");
...
assert(packageJson.version==="13.0.0"&&packageLock.version==="13.0.0","package version mismatch");
assert(packageJson.scripts?.quality==="node tests/validate-v13-0-0.mjs","quality script mismatch");
assert(readme.startsWith("# My Finance Records · V13.0.0 PWA"),"README heading mismatch");
...
assert(changelog.includes("## 13.0.0 · 2026-08-06"),"CHANGELOG V13 entry missing");
...
const syntaxFiles=[
  "security-profiles.js","cloud-sync.js","account-ledger.js","budget-planning.js","reports-insights.js","productivity-tools.js","reminders-alerts.js","sw.js","tests/validate-v13-0-0.mjs"
];
...
inlineScripts.forEach((code,index)=>{
  const temp=path.join(root,`.v13000-inline-${index}.js`);
...
if(failures.length){
  console.error("V13.0.0 Major Cloud, Encryption & Profile Architecture validation failed:\n"+failures.map(item=>`- ${item}`).join("\n"));
  process.exit(1);
}
console.log("V13.0.0 Major Cloud, Encryption & Profile Architecture validation passed.");
