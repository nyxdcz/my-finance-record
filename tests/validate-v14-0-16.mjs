...
const installer=read("Install_V14_0_16.command");
...
assert(version.version==="14.0.16","version.json is not V14.0.16");
assert(version.cacheVersion==="finance-v14-20260812-v1416-live-ui-cache-r1","V14.0.16 cache identifier mismatch");
...
assert(html.includes('<title>My Finance Records · V14.0.16</title>'),"HTML title mismatch");
assert(html.includes('const APP_VERSION = "14.0.16";'),"HTML APP_VERSION mismatch");
assert(html.includes('const APP_RELEASE_NAME = "Live UI Cache Refresh";'),"HTML release name mismatch");
...
assert(html.includes('{"version":"V14.0.16","title":"Live UI Cache Refresh"'),"in-app V14.0.16 history entry missing");
...
assert(html.includes('./projects-calendar-v13.0.20.js?v=14.0.16')&&html.includes('./projects-calendar-v13.0.20.css?v=14.0.16'),"Project Agenda assets are not version-pinned in HTML");
assert(worker.includes('asset("./projects-calendar-v13.0.20.js?v=14.0.16")')&&worker.includes('asset("./projects-calendar-v13.0.20.css?v=14.0.16")'),"Project Agenda assets are not version-pinned in the offline shell");
assert(html.includes('./cloud-conflict-review.js?v=14.0.16')&&worker.includes('asset("./cloud-conflict-review.js?v=14.0.16")'),"Cloud conflict review module is not loaded and precached");
assert(html.includes('./cloud-conflict-resolution.js?v=14.0.16')&&worker.includes('asset("./cloud-conflict-resolution.js?v=14.0.16")'),"Cloud conflict resolution module is not loaded and precached");
...
assert(read("index.html").includes('./app.css?v=14.0.16')&&worker.includes('asset("./app.css?v=14.0.16")'),"extracted application CSS is not versioned and precached");
...
assert(worker.includes('const APP_VERSION = "14.0.16";'),"service-worker version mismatch");
assert(worker.includes(`const CACHE_VERSION = "${version.cacheVersion}";`),"service-worker cache mismatch");
assert(worker.includes('asset("./security-profiles.js?v=14.0.16")')&&worker.includes('asset("./security-profiles.css?v=14.0.16")'),"profile assets missing from PWA shell");
...
assert(packageJson.version==="14.0.16"&&packageLock.version==="14.0.16","package version mismatch");
...
assert(worker.includes('asset("./privacy-lock.js?v=14.0.16")'),"privacy module missing from service-worker shell");
assert(workflow.includes("privacy-lock.js"),"GitHub Pages workflow does not deploy privacy module");
assert(exists("SIGNED_OUT_PRIVACY_LOCK_VALIDATION_V13_0_18.md"),"V14.0.16 privacy validation report missing");
...
const recentReadmeVersions=["V14.0.16","V14.0.15","V14.0.14","V14.0.13","V14.0.12"];
...
assert(exists("Install_V14_0_16.command")&&(fs.statSync(path.join(root,"Install_V14_0_16.command")).mode&0o100)!==0,"V14.0.16 installer is missing or not executable");
assert((fs.statSync(path.join(root,"run_audit.sh")).mode&0o100)!==0,"run_audit.sh is not executable");
assert(installer.includes("npm ci --ignore-scripts")&&installer.includes("npm run quality")&&installer.includes("git diff --check"),"V14.0.16 installer validation flow is incomplete");
...
assert(readme.startsWith("# My Finance Records · V14.0.16"),"README release heading mismatch");
...
const cloudSandbox={
  ...
  data:structuredClone(sampleData),APP_VERSION:"14.0.16",normalizeData:value=>value,saveData(){return true;},renderAll(){},showToast(message,tone){cloudSandbox.__lastToast={message,tone};},STORAGE_KEY:"finance",setInterval(){return 0;},clearInterval(){},setTimeout(){return 0;},clearTimeout(){}
};
...
assert(version.cacheVersion.includes("v1416-live-ui-cache-r1"),"V14.0.16 cache revision mismatch");
...
assert(html.includes('id="settingsOverviewAppStatus">Version 14.0.16<'),"Settings overview version was not updated");
...
assert(version.name==="Live UI Cache Refresh", "V14.0.16 version name mismatch");
...
assert(version.cacheVersion.includes("v1416-live-ui-cache-r1"),"V14.0.16 cache revision mismatch");
assert(readme.startsWith("# My Finance Records · V14.0.16"),"V14.0.16 README metadata missing");
assert(changelog.includes("## 13.0.15 · 2026-08-07"),"V13.0.15 changelog missing");
assert(html.includes('?v=14.0.16')&&worker.includes('?v=14.0.16'),"Version-pinned V13.0.15 assets missing");
...
]) assert(html.includes(token),`V14.0.16 auth UI safeguard missing: ${token}`);
for(const token of [
...
]) assert(cloud.includes(token),`V14.0.16 auth behavior safeguard missing: ${token}`);
assert(changelog.includes('Forgot password')&&changelog.includes('Test cloud connection'),"CHANGELOG auth recovery notes missing");
for(const token of ['id="cloudRecoveryHelpCard"','id="cloudRecoveryEmail"','id="cloudRecoveryCode"','id="cloudRecoveryResend"','id="cloudVerifyRecoveryCode"','id="cloudRecoveryBackToSignIn"']) assert(html.includes(token),`V14.0.16 recovery redirect UI safeguard missing: ${token}`);
for(const token of ['parsePasswordRecoveryUrl','recoveryErrorMessage','verifyRecoveryCode','verifyOtp({ email:value, token:code, type:"recovery" })','url.searchParams.set("auth", "recovery")','Password reset needs attention']) assert(cloud.includes(token),`V14.0.16 recovery redirect behavior safeguard missing: ${token}`);
...
assert(changelog.includes('## 14.0.16 · 2026-08-12'),"V14.0.16 changelog entry missing");
assert(version.name==='Live UI Cache Refresh',"V14.0.16 version name mismatch");
assert(changelog.includes('## 13.0.15 · 2026-08-07')&&changelog.includes('Password-reset messaging avoids revealing'),"CHANGELOG V14.0.16 auth entry missing");
assert(version.cacheVersion.includes('v1416-live-ui-cache-r1'),"V14.0.16 auth cache revision mismatch");
assert(packageJson.scripts?.quality==='npm run inspect && npm run lint && npm run maintainability && npm run test',"V14.0.16 quality script mismatch");


// V14.0.16 iPhone input zoom prevention safeguards.
assert(version.name==="Live UI Cache Refresh","V14.0.16 version name mismatch");
assert(version.cacheVersion.includes("v1416-live-ui-cache-r1"),"V14.0.16 cache revision mismatch");
assert(read("IPHONE_INPUT_ZOOM_VALIDATION_V13_0_17.md").includes("16px"),"V14.0.16 input-zoom guidance missing");
...
]) assert(html.includes(token),`V14.0.16 phone input safeguard missing: ${token}`);
assert(!/<meta[^>]+name=["']viewport["'][^>]+user-scalable\s*=\s*no/i.test(html),"V14.0.16 must not disable user pinch zoom");
assert(!/<meta[^>]+name=["']viewport["'][^>]+maximum-scale\s*=\s*1/i.test(html),"V14.0.16 must not lock maximum zoom scale");
assert(exists("IPHONE_INPUT_ZOOM_VALIDATION_V13_0_17.md"),"V14.0.16 input zoom validation report missing");

// V14.0.16 UI accessibility and signed-out Settings reliability safeguards.
for(const token of [
...
]) assert(html.includes(token),`V14.0.16 visual safeguard missing: ${token}`);
for(const token of [
...
]) assert(html.includes(token),`V14.0.16 interface safeguard missing: ${token}`);
for(const token of [
  "[data-help-key]",
...
]) assert(privacyLock.includes(token),`V14.0.16 privacy safeguard missing: ${token}`);
...
]) assert(browserCoverage.includes(token),`V14.0.16 browser coverage missing: ${token}`);
assert(readme.includes("**V14.0.16 · Live UI Cache Refresh**"),"README V14.0.16 release notes missing");
for(const token of [
...
]) assert(html.includes(token),`V14.0.16 history or Settings search safeguard missing: ${token}`);
for(const token of ["baseRedoLastChange", 'key === "y"', 'key === "z" && event.shiftKey']) assert(productivity.includes(token),`V14.0.16 redo shortcut safeguard missing: ${token}`);
for(const token of ['#settingsSearchButton', '#settingsSearchInput', '[data-settings-search-result]']) assert(privacyLock.includes(token),`V14.0.16 signed-out Settings search safeguard missing: ${token}`);

// V14.0.16 approved Dashboard arrangement and temporary compact-rail expansion.
...
]) assert(html.includes(token),`V14.0.16 Dashboard or rail behavior missing: ${token}`);
...
assert(changelog.includes("approved Dashboard card sequence")&&changelog.includes("saved pin state"),"V14.0.16 changelog scope is incomplete");

// V14.0.16 primary navigation labels and Philippine-time appearance schedule.
for(const [page,label] of Object.entries({dashboard:"Overview",money:"Finance",projects:"Work",reports:"Insights"})) {
  assert(html.includes(`data-page="${page}" data-nav-label="${label}"`),`V14.0.16 sidebar label missing for ${page}`);
}
for(const token of [
...
]) assert(html.includes(token),`V14.0.16 Philippine-time theme safeguard missing: ${token}`);
...
assert(changelog.includes("Asia/Manila")&&changelog.includes("7:01 AM")&&changelog.includes("6:00 PM"),"V14.0.16 Manila schedule documentation is incomplete");

// V14.0.16 duplicate sidebar group-heading removal.
...
]) assert(browserCoverage.includes(token),`V14.0.16 responsive sidebar coverage missing: ${token}`);
assert(changelog.includes("duplicated uppercase Overview, Finance, Work, and Insights headings"),"V14.0.16 sidebar cleanup documentation is incomplete");

// V14.0.16 accessible Dashboard interaction system.
...
assert(changelog.includes("one-week Dashboard calendar marquee")&&changelog.includes("ARIA live announcements")&&changelog.includes('aria-haspopup="menu"'),"V14.0.16 changelog scope is incomplete");

if(failures.length){
  console.error("V14.0.16 Live UI Cache Refresh validation failed:\n"+failures.map(item=>`- ${item}`).join("\n"));
  process.exit(1);
}
console.log("V14.0.16 Live UI Cache Refresh validation passed.");
