...
assert(version.version==="13.0.3","version.json is not V13.0.3");
...
assert(html.includes('<title>My Finance Records · V13.0.3</title>'),"HTML title mismatch");
assert(html.includes('const APP_VERSION = "13.0.3";'),"HTML APP_VERSION mismatch");
...
assert(worker.includes('const APP_VERSION = "13.0.3";'),"service-worker version mismatch");
...
assert(packageJson.version==="13.0.3"&&packageLock.version==="13.0.3","package version mismatch");
assert(packageJson.scripts?.quality==="node tests/validate-v13-0-3.mjs","quality script mismatch");
...
assert(readme.startsWith("# My Finance Records · V13.0.3 PWA"),"README heading mismatch");
...
assert(changelog.includes("## 13.0.3 · 2026-08-07"),"CHANGELOG V13 entry missing");
...
const syntaxFiles=[
  "security-profiles.js","cloud-sync.js","account-ledger.js","budget-planning.js","reports-insights.js","productivity-tools.js","reminders-alerts.js","sw.js","tests/validate-v13-0-3.mjs"
];
...
assert(version.cacheVersion.includes("v13003-compact-modals-svg-r1"),"Settings cache revision mismatch");

// V13.0.3 compact modal and SVG icon safeguards.
for(const token of [
...
assert(compactModalValidation.includes("3 × 2 desktop grid")&&compactModalValidation.includes("44px touch targets")&&compactModalValidation.includes("No finance formula"),"V13.0.3 validation scope incomplete");
...
assert(html.includes('id="settingsOverviewAppStatus">Version 13.0.3<'),"Settings overview version was not updated");
...
if(failures.length){
  console.error("V13.0.3 Compact Modal & SVG Icon validation failed:\n"+failures.map(item=>`- ${item}`).join("\n"));
  process.exit(1);
}
console.log("V13.0.3 Compact Modal & SVG Icon validation passed.");
