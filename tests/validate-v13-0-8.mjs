...
assert(version.version==="13.0.8","version.json is not V13.0.8");
...
assert(html.includes('<title>My Finance Records · V13.0.8</title>'),"HTML title mismatch");
assert(html.includes('const APP_VERSION = "13.0.8";'),"HTML APP_VERSION mismatch");
...
assert(html.includes('{"version": "V13.0.8", "title": "Calendar Paid-State & Project Completion Cleanup"'),"in-app V13.0.8 history entry missing");
...
assert(worker.includes('const APP_VERSION = "13.0.8";'),"service-worker version mismatch");
...
assert(packageJson.version==="13.0.8"&&packageLock.version==="13.0.8","package version mismatch");
assert(packageJson.scripts?.quality==="node tests/validate-v13-0-8.mjs","quality script mismatch");
...
assert(exists("PROJECT_REVISION_CYCLES_VALIDATION_V13_0_6.md"),"V13.0.8 project revision validation report missing");
assert(exists("FILE_INSPECTION_AND_FIXES_VALIDATION_V13_0_5.md"),"V13.0.5 installer validation report missing");
assert(readme.includes("My_Finance_Records_V13_0_8_File_Inspection_and_Fixes_Installer.zip")&&readme.includes("Install_V13_0_8.command"),"README V13.0.8 installer filenames are stale");
...
assert(readme.startsWith("# My Finance Records · V13.0.8 PWA"),"README heading mismatch");
...
assert(changelog.includes("## 13.0.8 · 2026-08-07"),"CHANGELOG V13.0.8 entry missing");
...
const syntaxFiles=[
  "security-profiles.js","cloud-sync.js","account-ledger.js","budget-planning.js","reports-insights.js","productivity-tools.js","reminders-alerts.js","sw.js","tests/validate-v13-0-8.mjs"
];
...
assert(version.cacheVersion.includes("v13008-paid-calendar-project-completion-r1"),"V13.0.8 cache revision mismatch");
...
// V13.0.5 Dashboard calendar deduplication safeguards retained in V13.0.8.
for(const token of [
...
]) assert(html.includes(token),`V13.0.5 calendar safeguard missing after V13.0.8: ${token}`);
...
// V13.0.8 Project revision-cycle safeguards.
for(const token of [
...
]) assert(html.includes(token),`V13.0.8 project revision safeguard missing: ${token}`);
assert(html.includes('<option>In revision</option>'),"In revision project status option missing");
assert(changelog.includes("numbered project revision cycles")&&readme.includes("Revision 1, Revision 2"),"V13.0.8 project revision documentation incomplete");
assert(exists("PROJECT_REVISION_CYCLES_VALIDATION_V13_0_6.md"),"V13.0.8 project revision report missing");
assert(exists("FILE_INSPECTION_AND_FIXES_VALIDATION_V13_0_6.md"),"V13.0.6 installer validation report missing");

assert(html.includes('id="settingsOverviewAppStatus">Version 13.0.8<'),"Settings overview version was not updated");
...
// V13.0.8 Monthly budget-plan compact-layout safeguards.
...
]) assert(budgetSource.includes(token),`V13.0.8 budget-plan collapse safeguard missing: ${token}`);
for (const token of [
...
]) assert(budgetCss.includes(token),`V13.0.8 compact budget CSS safeguard missing: ${token}`);
...
assert(exists("BUDGET_PLAN_COMPACT_LAYOUT_VALIDATION_V13_0_7.md"),"V13.0.8 budget layout validation report missing");
assert(exists("FILE_INSPECTION_AND_FIXES_VALIDATION_V13_0_7.md"),"V13.0.8 installer validation report missing");
assert(version.name==="Calendar Paid-State & Project Completion Cleanup","V13.0.8 version name mismatch");

// V13.0.8 paid-calendar, project completion, and disclosure consistency safeguards.
assert(html.includes('if (paid) return;')&&html.includes('status:"Due"')&&!html.includes('status:paid ? "Paid" : "Due"'),"V13.0.8 paid expenses are still projected as Dashboard calendar due-events");
assert(html.includes('const active = filtered.filter(project => project.status !== "Completed")')&&html.includes('const completed = filtered.filter(project => project.status === "Completed")'),"V13.0.8 project lifecycle grouping is missing");
assert(html.includes('Completed · balance due')&&html.includes('Mark paid')&&html.includes('>Revise</button>')&&html.includes('>Finish revision</button>'),"V13.0.8 completed-balance or compact project actions missing");
for (const token of [
...
]) assert(html.includes(token),`V13.0.8 disclosure/action UI safeguard missing: ${token}`);
assert(!html.includes('<span class="collapse-icon" aria-hidden="true">▾</span>'),"Legacy text-glyph collapse chevrons remain");
assert(readme.includes("V13.0.8 · Calendar Paid-State & Project Completion Cleanup")&&changelog.includes("Completed Projects whether fully paid or still carrying a client balance"),"V13.0.8 documentation incomplete");
assert(exists("CALENDAR_PAID_PROJECT_COMPLETION_VALIDATION_V13_0_8.md"),"V13.0.8 validation report missing");
assert(exists("FILE_INSPECTION_AND_FIXES_VALIDATION_V13_0_8.md"),"V13.0.8 installer validation report missing");
assert(version.name==="Calendar Paid-State & Project Completion Cleanup","V13.0.8 version name mismatch");

if(failures.length){
  console.error("V13.0.8 Calendar Paid-State & Project Completion Cleanup validation failed:\n"+failures.map(item=>`- ${item}`).join("\n"));
  process.exit(1);
}
console.log("V13.0.8 Calendar Paid-State & Project Completion Cleanup validation passed.");
