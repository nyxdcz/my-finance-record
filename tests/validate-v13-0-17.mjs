...
assert(version.version==="13.0.17","version.json is not V13.0.17");
...
assert(html.includes('<title>My Finance Records · V13.0.17</title>'),"HTML title mismatch");
assert(html.includes('const APP_VERSION = "13.0.17";'),"HTML APP_VERSION mismatch");
...
assert(worker.includes('const APP_VERSION = "13.0.17";'),"service-worker version mismatch");
assert(worker.includes(`const CACHE_VERSION = "${version.cacheVersion}";`),"service-worker cache mismatch");
assert(worker.includes('asset("./security-profiles.js?v=13.0.17")')&&worker.includes('asset("./security-profiles.css?v=13.0.17")'),"profile assets missing from PWA shell");
...
assert(packageJson.version==="13.0.17"&&packageLock.version==="13.0.17","package version mismatch");
assert(packageJson.scripts?.quality==="node tests/validate-v13-0-17.mjs","quality script mismatch");
...
assert(readme.includes("My_Finance_Records_V13_0_17_File_Inspection_and_Fixes_Installer.zip")&&readme.includes("Install_V13_0_17.command"),"README V13.0.15 installer filenames are stale");
...
assert(readme.startsWith("# My Finance Records · V13.0.17 PWA"),"README V13.0.15 heading mismatch");
...
const syntaxFiles=[
  "security-profiles.js","cloud-sync.js","account-ledger.js","budget-planning.js","reports-insights.js","productivity-tools.js","reminders-alerts.js","sw.js","tests/validate-v13-0-10.mjs"
];
...
assert(version.cacheVersion.includes("v13017-iphone-input-zoom-r1"),"V13.0.15 cache revision mismatch");
...
assert(html.includes('id="settingsOverviewAppStatus">Version 13.0.17<'),"Settings overview version was not updated");
...
assert(version.name==="iPhone Input Zoom Prevention", "V13.0.15 version name mismatch");
...
assert(version.cacheVersion.includes("v13017-iphone-input-zoom-r1"),"V13.0.15 cache revision mismatch");
assert(readme.startsWith("# My Finance Records · V13.0.17 PWA")&&readme.includes("Brave PWA Install Flow"),"V13.0.15 README metadata missing");
assert(changelog.includes("## 13.0.15 · 2026-08-07"),"V13.0.15 changelog missing");
assert(html.includes('?v=13.0.17')&&worker.includes('?v=13.0.17'),"Version-pinned V13.0.15 assets missing");
...
]) assert(html.includes(token),`V13.0.17 auth UI safeguard missing: ${token}`);
for(const token of [
  ...
  'testCloudConnection','/auth/v1/health','withAuthButtonBusy','setPasswordVisibility','Wrong email or password','local finance records are still available'
]) assert(cloud.includes(token),`V13.0.17 auth behavior safeguard missing: ${token}`);
assert(readme.includes('Forgot password?')&&readme.includes('Test cloud connection'),"README V13.0.17 auth recovery notes missing");
for(const token of ['id="cloudRecoveryHelpCard"','id="cloudRecoveryEmail"','id="cloudRecoveryCode"','id="cloudRecoveryResend"','id="cloudVerifyRecoveryCode"','id="cloudRecoveryBackToSignIn"']) assert(html.includes(token),`V13.0.17 recovery redirect UI safeguard missing: ${token}`);
for(const token of ['parsePasswordRecoveryUrl','recoveryErrorMessage','verifyRecoveryCode','verifyOtp({ email:value, token:code, type:"recovery" })','url.searchParams.set("auth", "recovery")','Password reset needs attention']) assert(cloud.includes(token),`V13.0.17 recovery redirect behavior safeguard missing: ${token}`);
assert(readme.includes('?auth=recovery')&&readme.includes('{{ .Token }}'),"README V13.0.17 recovery redirect/code instructions missing");
assert(changelog.includes('## 13.0.17 · 2026-08-07'),"V13.0.17 changelog entry missing");
assert(version.name==='iPhone Input Zoom Prevention',"V13.0.17 version name mismatch");
assert(changelog.includes('## 13.0.15 · 2026-08-07')&&changelog.includes('Password-reset messaging avoids revealing'),"CHANGELOG V13.0.17 auth entry missing");
assert(version.cacheVersion.includes('v13017-iphone-input-zoom-r1'),"V13.0.17 auth cache revision mismatch");
assert(packageJson.scripts?.quality==='node tests/validate-v13-0-17.mjs',"V13.0.17 quality script mismatch");


// V13.0.17 iPhone input zoom prevention safeguards.
assert(version.name==="iPhone Input Zoom Prevention","V13.0.17 version name mismatch");
assert(version.cacheVersion.includes("v13017-iphone-input-zoom-r1"),"V13.0.17 cache revision mismatch");
assert(readme.includes("iPhone Input Zoom Prevention")&&readme.includes("minimum **16px**"),"README V13.0.17 input-zoom guidance missing");
...
]) assert(html.includes(token),`V13.0.17 phone input safeguard missing: ${token}`);
assert(!/<meta[^>]+name=["']viewport["'][^>]+user-scalable\s*=\s*no/i.test(html),"V13.0.17 must not disable user pinch zoom");
assert(!/<meta[^>]+name=["']viewport["'][^>]+maximum-scale\s*=\s*1/i.test(html),"V13.0.17 must not lock maximum zoom scale");
assert(exists("IPHONE_INPUT_ZOOM_VALIDATION_V13_0_17.md"),"V13.0.17 input zoom validation report missing");

if(failures.length){
  console.error("V13.0.17 iPhone Input Zoom Prevention validation failed:\n"+failures.map(item=>`- ${item}`).join("\n"));
  process.exit(1);
}
console.log("V13.0.17 iPhone Input Zoom Prevention validation passed.");
