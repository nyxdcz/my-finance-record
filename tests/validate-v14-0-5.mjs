...
const installer=read("Install_V14_0_5.command");
...
assert(version.version==="14.0.5","version.json is not V14.0.5");
...
assert(html.includes('<title>My Finance Records · V14.0.5</title>'),"HTML title mismatch");
assert(html.includes('const APP_VERSION = "14.0.5";'),"HTML APP_VERSION mismatch");
assert(html.includes('const APP_RELEASE_NAME = "Desktop Navigation Consistency";'),"HTML release name mismatch");
...
assert(html.includes('{"version":"V14.0.5","title":"Desktop Navigation Consistency"'),"in-app V14.0.5 history entry missing");
...
assert(html.includes('./projects-calendar-v13.0.20.js?v=14.0.5')&&html.includes('./projects-calendar-v13.0.20.css?v=14.0.5'),"Project Agenda assets are not version-pinned in HTML");
assert(worker.includes('asset("./projects-calendar-v13.0.20.js?v=14.0.5")')&&worker.includes('asset("./projects-calendar-v13.0.20.css?v=14.0.5")'),"Project Agenda assets are not version-pinned in the offline shell");
assert(html.includes('./cloud-conflict-review.js?v=14.0.5')&&worker.includes('asset("./cloud-conflict-review.js?v=14.0.5")'),"Cloud conflict review module is not loaded and precached");
...
assert(read("index.html").includes('./app.css?v=14.0.5')&&worker.includes('asset("./app.css?v=14.0.5")'),"extracted application CSS is not versioned and precached");
...
assert(worker.includes('const APP_VERSION = "14.0.5";'),"service-worker version mismatch");
assert(worker.includes(`const CACHE_VERSION = "${version.cacheVersion}";`),"service-worker cache mismatch");
assert(worker.includes('asset("./security-profiles.js?v=14.0.5")')&&worker.includes('asset("./security-profiles.css?v=14.0.5")'),"profile assets missing from PWA shell");
...
assert(packageJson.version==="14.0.5"&&packageLock.version==="14.0.5","package version mismatch");
...
assert(worker.includes('asset("./privacy-lock.js?v=14.0.5")'),"privacy module missing from service-worker shell");
assert(workflow.includes("privacy-lock.js"),"GitHub Pages workflow does not deploy privacy module");
assert(exists("SIGNED_OUT_PRIVACY_LOCK_VALIDATION_V13_0_18.md"),"V14.0.5 privacy validation report missing");
...
assert(readme.includes("macOS File Inspection and Fixes installer")&&readme.includes("Install_V14_0_5.command"),"README installer instructions missing");
...
assert(exists("Install_V14_0_5.command")&&(fs.statSync(path.join(root,"Install_V14_0_5.command")).mode&0o100)!==0,"V14.0.5 installer is missing or not executable");
assert((fs.statSync(path.join(root,"run_audit.sh")).mode&0o100)!==0,"run_audit.sh is not executable");
assert(installer.includes("npm ci --ignore-scripts")&&installer.includes("npm run quality")&&installer.includes("git diff --check"),"V14.0.5 installer validation flow is incomplete");
...
assert(readme.startsWith("# My Finance Records · V14.0.5 PWA"),"README V13.0.15 heading mismatch");
...
assert(version.cacheVersion.includes("v1405-navigation-consistency-r1"),"V13.0.15 cache revision mismatch");
...
assert(html.includes('id="settingsOverviewAppStatus">Version 14.0.5<'),"Settings overview version was not updated");
...
assert(version.name==="Desktop Navigation Consistency", "V13.0.15 version name mismatch");
...
assert(version.cacheVersion.includes("v1405-navigation-consistency-r1"),"V13.0.15 cache revision mismatch");
assert(readme.startsWith("# My Finance Records · V14.0.5 PWA")&&readme.includes("Brave PWA Install Flow"),"V13.0.15 README metadata missing");
assert(changelog.includes("## 13.0.15 · 2026-08-07"),"V13.0.15 changelog missing");
assert(html.includes('?v=14.0.5')&&worker.includes('?v=14.0.5'),"Version-pinned V13.0.15 assets missing");
...
]) assert(html.includes(token),`V14.0.5 auth UI safeguard missing: ${token}`);
for(const token of [
...
]) assert(cloud.includes(token),`V14.0.5 auth behavior safeguard missing: ${token}`);
assert(readme.includes('Forgot password?')&&readme.includes('Test cloud connection'),"README V14.0.5 auth recovery notes missing");
for(const token of ['id="cloudRecoveryHelpCard"','id="cloudRecoveryEmail"','id="cloudRecoveryCode"','id="cloudRecoveryResend"','id="cloudVerifyRecoveryCode"','id="cloudRecoveryBackToSignIn"']) assert(html.includes(token),`V14.0.5 recovery redirect UI safeguard missing: ${token}`);
for(const token of ['parsePasswordRecoveryUrl','recoveryErrorMessage','verifyRecoveryCode','verifyOtp({ email:value, token:code, type:"recovery" })','url.searchParams.set("auth", "recovery")','Password reset needs attention']) assert(cloud.includes(token),`V14.0.5 recovery redirect behavior safeguard missing: ${token}`);
assert(readme.includes('?auth=recovery')&&readme.includes('{{ .Token }}'),"README V14.0.5 recovery redirect/code instructions missing");
assert(changelog.includes('## 14.0.5 · 2026-08-11'),"V14.0.5 changelog entry missing");
assert(version.name==='Desktop Navigation Consistency',"V14.0.5 version name mismatch");
assert(changelog.includes('## 13.0.15 · 2026-08-07')&&changelog.includes('Password-reset messaging avoids revealing'),"CHANGELOG V14.0.5 auth entry missing");
assert(version.cacheVersion.includes('v1405-navigation-consistency-r1'),"V14.0.5 auth cache revision mismatch");
assert(packageJson.scripts?.quality==='npm run inspect && npm run lint && npm run maintainability && npm run test',"V14.0.5 quality script mismatch");


// V14.0.5 iPhone input zoom prevention safeguards.
assert(version.name==="Desktop Navigation Consistency","V14.0.5 version name mismatch");
assert(version.cacheVersion.includes("v1405-navigation-consistency-r1"),"V14.0.5 cache revision mismatch");
assert(readme.includes("iPhone Input Zoom Prevention")&&readme.includes("minimum **16px**"),"README V14.0.5 input-zoom guidance missing");
...
]) assert(html.includes(token),`V14.0.5 phone input safeguard missing: ${token}`);
assert(!/<meta[^>]+name=["']viewport["'][^>]+user-scalable\s*=\s*no/i.test(html),"V14.0.5 must not disable user pinch zoom");
assert(!/<meta[^>]+name=["']viewport["'][^>]+maximum-scale\s*=\s*1/i.test(html),"V14.0.5 must not lock maximum zoom scale");
assert(exists("IPHONE_INPUT_ZOOM_VALIDATION_V13_0_17.md"),"V14.0.5 input zoom validation report missing");

if(failures.length){
  console.error("V14.0.5 Desktop Navigation Consistency validation failed:\n"+failures.map(item=>`- ${item}`).join("\n"));
  process.exit(1);
}
console.log("V14.0.5 Desktop Navigation Consistency validation passed.");
