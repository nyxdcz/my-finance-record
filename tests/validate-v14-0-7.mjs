...
const installer=read("Install_V14_0_7.command");
...
assert(version.version==="14.0.7","version.json is not V14.0.7");
...
assert(html.includes('<title>My Finance Records · V14.0.7</title>'),"HTML title mismatch");
assert(html.includes('const APP_VERSION = "14.0.7";'),"HTML APP_VERSION mismatch");
assert(html.includes('const APP_RELEASE_NAME = "Cloud Conflict Resolution Reliability";'),"HTML release name mismatch");
...
assert(html.includes('{"version":"V14.0.7","title":"Cloud Conflict Resolution Reliability"'),"in-app V14.0.7 history entry missing");
...
assert(html.includes('./projects-calendar-v13.0.20.js?v=14.0.7')&&html.includes('./projects-calendar-v13.0.20.css?v=14.0.7'),"Project Agenda assets are not version-pinned in HTML");
assert(worker.includes('asset("./projects-calendar-v13.0.20.js?v=14.0.7")')&&worker.includes('asset("./projects-calendar-v13.0.20.css?v=14.0.7")'),"Project Agenda assets are not version-pinned in the offline shell");
assert(html.includes('./cloud-conflict-review.js?v=14.0.7')&&worker.includes('asset("./cloud-conflict-review.js?v=14.0.7")'),"Cloud conflict review module is not loaded and precached");
assert(html.includes('./cloud-conflict-resolution.js?v=14.0.7')&&worker.includes('asset("./cloud-conflict-resolution.js?v=14.0.7")'),"Cloud conflict resolution module is not loaded and precached");
...
assert(read("index.html").includes('./app.css?v=14.0.7')&&worker.includes('asset("./app.css?v=14.0.7")'),"extracted application CSS is not versioned and precached");
...
assert(worker.includes('const APP_VERSION = "14.0.7";'),"service-worker version mismatch");
assert(worker.includes(`const CACHE_VERSION = "${version.cacheVersion}";`),"service-worker cache mismatch");
assert(worker.includes('asset("./security-profiles.js?v=14.0.7")')&&worker.includes('asset("./security-profiles.css?v=14.0.7")'),"profile assets missing from PWA shell");
...
assert(packageJson.version==="14.0.7"&&packageLock.version==="14.0.7","package version mismatch");
...
assert(worker.includes('asset("./privacy-lock.js?v=14.0.7")'),"privacy module missing from service-worker shell");
assert(workflow.includes("privacy-lock.js"),"GitHub Pages workflow does not deploy privacy module");
assert(exists("SIGNED_OUT_PRIVACY_LOCK_VALIDATION_V13_0_18.md"),"V14.0.7 privacy validation report missing");
...
assert(readme.includes("macOS File Inspection and Fixes installer")&&readme.includes("Install_V14_0_7.command"),"README installer instructions missing");
...
assert(exists("Install_V14_0_7.command")&&(fs.statSync(path.join(root,"Install_V14_0_7.command")).mode&0o100)!==0,"V14.0.7 installer is missing or not executable");
assert((fs.statSync(path.join(root,"run_audit.sh")).mode&0o100)!==0,"run_audit.sh is not executable");
assert(installer.includes("npm ci --ignore-scripts")&&installer.includes("npm run quality")&&installer.includes("git diff --check"),"V14.0.7 installer validation flow is incomplete");
...
assert(readme.startsWith("# My Finance Records · V14.0.7 PWA"),"README V13.0.15 heading mismatch");
...
const cloudQueueFixture={
  ...
  [cloudChoiceKey]:{key:cloudChoiceKey,collection:"projects",recordId:"project-cloud-choice",payload:{id:"project-cloud-choice",name:"Device edit"},basePayload:{id:"project-cloud-choice",name:"Old base"},sortIndex:2,baseSortIndex:2,baseRevision:3,status:"conflict"},
  [deviceChoiceKey]:{key:deviceChoiceKey,collection:"projects",recordId:"project-device-choice",payload:{id:"project-device-choice",name:"Keep device edit"},basePayload:{id:"project-device-choice",name:"Old base"},sortIndex:4,baseSortIndex:4,baseRevision:6,status:"conflict"}
};
...
const cloudSandbox={
  ...
  data:structuredClone(sampleData),APP_VERSION:"14.0.7",normalizeData:value=>value,saveData(){return true;},renderAll(){},showToast(message,tone){cloudSandbox.__lastToast={message,tone};},STORAGE_KEY:"finance",setInterval(){return 0;},clearInterval(){},setTimeout(){return 0;},clearTimeout(){}
};
...
if(cloudInternals){
  ...
  assert(cloudSandbox.FinanceCloudSync.status.pendingCount===2&&cloudSandbox.FinanceCloudSync.status.conflictCount===2,"settings repair changed unrelated conflict records");
  ...
  assert(cloudResolution.choice==="cloud"&&baseAfterCloud[cloudChoiceKey]?.payload?.name==="Cloud edit"&&baseAfterCloud[cloudChoiceKey]?.revision===8&&baseAfterCloud[cloudChoiceKey]?.sortIndex===5,"Use cloud version did not persist the reviewed cloud record");
  ...
  assert(deviceResolution.choice==="device"&&baseAfterDevice[deviceChoiceKey]?.payload?.name==="New cloud base"&&baseAfterDevice[deviceChoiceKey]?.revision===11,"Use this device did not persist the reviewed cloud base");
  assert(queueAfterDevice[deviceChoiceKey]?.payload?.name==="Keep device edit"&&queueAfterDevice[deviceChoiceKey]?.baseRevision===11&&queueAfterDevice[deviceChoiceKey]?.status==="pending","Use this device did not rebase and queue the local record");
...
assert(version.cacheVersion.includes("v1407-cloud-conflict-resolution-r1"),"V13.0.15 cache revision mismatch");
...
assert(html.includes('id="settingsOverviewAppStatus">Version 14.0.7<'),"Settings overview version was not updated");
...
assert(version.name==="Cloud Conflict Resolution Reliability", "V13.0.15 version name mismatch");
...
assert(version.cacheVersion.includes("v1407-cloud-conflict-resolution-r1"),"V13.0.15 cache revision mismatch");
assert(readme.startsWith("# My Finance Records · V14.0.7 PWA")&&readme.includes("Brave PWA Install Flow"),"V13.0.15 README metadata missing");
assert(changelog.includes("## 13.0.15 · 2026-08-07"),"V13.0.15 changelog missing");
assert(html.includes('?v=14.0.7')&&worker.includes('?v=14.0.7'),"Version-pinned V13.0.15 assets missing");
...
]) assert(html.includes(token),`V14.0.7 auth UI safeguard missing: ${token}`);
for(const token of [
...
]) assert(cloud.includes(token),`V14.0.7 auth behavior safeguard missing: ${token}`);
assert(readme.includes('Forgot password?')&&readme.includes('Test cloud connection'),"README V14.0.7 auth recovery notes missing");
for(const token of ['id="cloudRecoveryHelpCard"','id="cloudRecoveryEmail"','id="cloudRecoveryCode"','id="cloudRecoveryResend"','id="cloudVerifyRecoveryCode"','id="cloudRecoveryBackToSignIn"']) assert(html.includes(token),`V14.0.7 recovery redirect UI safeguard missing: ${token}`);
for(const token of ['parsePasswordRecoveryUrl','recoveryErrorMessage','verifyRecoveryCode','verifyOtp({ email:value, token:code, type:"recovery" })','url.searchParams.set("auth", "recovery")','Password reset needs attention']) assert(cloud.includes(token),`V14.0.7 recovery redirect behavior safeguard missing: ${token}`);
assert(readme.includes('?auth=recovery')&&readme.includes('{{ .Token }}'),"README V14.0.7 recovery redirect/code instructions missing");
assert(changelog.includes('## 14.0.7 · 2026-08-12'),"V14.0.7 changelog entry missing");
assert(version.name==='Cloud Conflict Resolution Reliability',"V14.0.7 version name mismatch");
assert(changelog.includes('## 13.0.15 · 2026-08-07')&&changelog.includes('Password-reset messaging avoids revealing'),"CHANGELOG V14.0.7 auth entry missing");
assert(version.cacheVersion.includes('v1407-cloud-conflict-resolution-r1'),"V14.0.7 auth cache revision mismatch");
assert(packageJson.scripts?.quality==='npm run inspect && npm run lint && npm run maintainability && npm run test',"V14.0.7 quality script mismatch");


// V14.0.7 iPhone input zoom prevention safeguards.
assert(version.name==="Cloud Conflict Resolution Reliability","V14.0.7 version name mismatch");
assert(version.cacheVersion.includes("v1407-cloud-conflict-resolution-r1"),"V14.0.7 cache revision mismatch");
assert(readme.includes("iPhone Input Zoom Prevention")&&readme.includes("minimum **16px**"),"README V14.0.7 input-zoom guidance missing");
...
]) assert(html.includes(token),`V14.0.7 phone input safeguard missing: ${token}`);
assert(!/<meta[^>]+name=["']viewport["'][^>]+user-scalable\s*=\s*no/i.test(html),"V14.0.7 must not disable user pinch zoom");
assert(!/<meta[^>]+name=["']viewport["'][^>]+maximum-scale\s*=\s*1/i.test(html),"V14.0.7 must not lock maximum zoom scale");
assert(exists("IPHONE_INPUT_ZOOM_VALIDATION_V13_0_17.md"),"V14.0.7 input zoom validation report missing");

// V14.0.7 UI accessibility and signed-out Settings reliability safeguards.
for(const token of [
...
]) assert(html.includes(token),`V14.0.7 visual safeguard missing: ${token}`);
for(const token of [
  ...
  "Cloud Conflict Resolution Reliability"
]) assert(html.includes(token),`V14.0.7 interface safeguard missing: ${token}`);
for(const token of [
  "[data-help-key]",
...
]) assert(privacyLock.includes(token),`V14.0.7 privacy safeguard missing: ${token}`);
...
]) assert(browserCoverage.includes(token),`V14.0.7 browser coverage missing: ${token}`);
assert(readme.includes("## V14.0.7 · Cloud Conflict Resolution Reliability"),"README V14.0.7 release notes missing");

if(failures.length){
  console.error("V14.0.7 Cloud Conflict Resolution Reliability validation failed:\n"+failures.map(item=>`- ${item}`).join("\n"));
  process.exit(1);
}
console.log("V14.0.7 Cloud Conflict Resolution Reliability validation passed.");
