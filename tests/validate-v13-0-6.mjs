...
assert(version.version==="13.0.6","version.json is not V13.0.6");
...
assert(html.includes('<title>My Finance Records · V13.0.6</title>'),"HTML title mismatch");
assert(html.includes('const APP_VERSION = "13.0.6";'),"HTML APP_VERSION mismatch");
...
assert(html.includes('{"version": "V13.0.6", "title": "Project Revision Cycles"'),"in-app V13.0.6 history entry missing");
...
assert(worker.includes('const APP_VERSION = "13.0.6";'),"service-worker version mismatch");
...
assert(packageJson.version==="13.0.6"&&packageLock.version==="13.0.6","package version mismatch");
assert(packageJson.scripts?.quality==="node tests/validate-v13-0-6.mjs","quality script mismatch");
...
assert(exists("PROJECT_REVISION_CYCLES_VALIDATION_V13_0_6.md"),"V13.0.6 project revision validation report missing");
assert(exists("FILE_INSPECTION_AND_FIXES_VALIDATION_V13_0_5.md"),"V13.0.5 installer validation report missing");
assert(readme.includes("My_Finance_Records_V13_0_6_File_Inspection_and_Fixes_Installer.zip")&&readme.includes("Install_V13_0_6.command"),"README V13.0.6 installer filenames are stale");
...
assert(readme.startsWith("# My Finance Records · V13.0.6 PWA"),"README heading mismatch");
...
assert(changelog.includes("## 13.0.6 · 2026-08-07"),"CHANGELOG V13.0.6 entry missing");
...
const syntaxFiles=[
  "security-profiles.js","cloud-sync.js","account-ledger.js","budget-planning.js","reports-insights.js","productivity-tools.js","reminders-alerts.js","sw.js","tests/validate-v13-0-6.mjs"
];
...
assert(version.cacheVersion.includes("v13006-project-revisions-r1"),"V13.0.6 cache revision mismatch");
...
// V13.0.5 Dashboard calendar deduplication safeguards retained in V13.0.6.
for(const token of [
...
]) assert(html.includes(token),`V13.0.5 calendar safeguard missing after V13.0.6: ${token}`);
...
// V13.0.6 Project revision-cycle safeguards.
for(const token of [
...
]) assert(html.includes(token),`V13.0.6 project revision safeguard missing: ${token}`);
assert(html.includes('<option>In revision</option>'),"In revision project status option missing");
assert(changelog.includes("numbered project revision cycles")&&readme.includes("Revision 1, Revision 2"),"V13.0.6 project revision documentation incomplete");
assert(exists("PROJECT_REVISION_CYCLES_VALIDATION_V13_0_6.md"),"V13.0.6 project revision report missing");
assert(exists("FILE_INSPECTION_AND_FIXES_VALIDATION_V13_0_6.md"),"V13.0.6 installer validation report missing");

assert(html.includes('id="settingsOverviewAppStatus">Version 13.0.6<'),"Settings overview version was not updated");
...
if(failures.length){
  console.error("V13.0.6 Project Revision Cycles validation failed:\n"+failures.map(item=>`- ${item}`).join("\n"));
  process.exit(1);
}
console.log("V13.0.6 Project Revision Cycles validation passed.");
