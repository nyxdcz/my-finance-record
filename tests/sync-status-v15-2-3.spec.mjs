...
test("V15.2.6 Cloud Sync uses supplied state icons and matching colors", async ({ page }) => {
...
test("V15.2.6 release metadata is visible", async ({ page }) => {
  await page.goto("http://127.0.0.1:3000/index.html?page=dashboard", { waitUntil:"networkidle" });
  await expect(page).toHaveTitle(/V15\.2\.6/);
  await expect(page.locator("#buildBadge")).toContainText("V15.2.6");
});
