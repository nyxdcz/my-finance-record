...
assert(version.version==="13.0.9","version.json is not V13.0.9");
...
assert(html.includes('<title>My Finance Records · V13.0.9</title>'),"HTML title mismatch");
assert(html.includes('const APP_VERSION = "13.0.9";'),"HTML APP_VERSION mismatch");
...
assert(html.includes('{"version": "V13.0.9", "title": "Phone UI & Account Balance Refresh"'),"in-app V13.0.9 history entry missing");
...
assert(worker.includes('const APP_VERSION = "13.0.9";'),"service-worker version mismatch");
...
assert(packageJson.version==="13.0.9"&&packageLock.version==="13.0.9","package version mismatch");
assert(packageJson.scripts?.quality==="node tests/validate-v13-0-9.mjs","quality script mismatch");
...
assert(readme.includes("My_Finance_Records_V13_0_9_File_Inspection_and_Fixes_Installer.zip")&&readme.includes("Install_V13_0_9.command"),"README V13.0.9 installer filenames are stale");
...
assert(readme.startsWith("# My Finance Records · V13.0.9 PWA"),"README heading mismatch");
...
assert(changelog.includes("## 13.0.9 · 2026-08-07"),"CHANGELOG V13.0.9 entry missing");
...
const syntaxFiles=[
  "security-profiles.js","cloud-sync.js","account-ledger.js","budget-planning.js","reports-insights.js","productivity-tools.js","reminders-alerts.js","sw.js","tests/validate-v13-0-9.mjs"
];
...
assert(version.cacheVersion.includes("v13009-phone-account-project-r1"),"V13.0.9 cache revision mismatch");
...
assert(html.includes('id="settingsOverviewAppStatus">Version 13.0.9<'),"Settings overview version was not updated");
...
assert(version.name==="Phone UI & Account Balance Refresh","V13.0.9 version name mismatch");
...
assert(readme.includes("V13.0.9 · Phone UI & Account Balance Refresh")&&changelog.includes("## 13.0.9 · 2026-08-07"),"V13.0.9 documentation incomplete");
assert(exists("PHONE_UI_ACCOUNT_BALANCE_VALIDATION_V13_0_9.md"),"V13.0.9 validation report missing");

if(failures.length){
  console.error("V13.0.9 Phone UI & Account Balance Refresh validation failed:\n"+failures.map(item=>`- ${item}`).join("\n"));
  process.exit(1);
}
console.log("V13.0.9 Phone UI & Account Balance Refresh validation passed.");
