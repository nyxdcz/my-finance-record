...
assert(version.version==="13.0.18","version.json is not V13.0.18");
...
assert(html.includes('<title>My Finance Records · V13.0.18</title>'),"HTML title mismatch");
assert(html.includes('const APP_VERSION = "13.0.18";'),"HTML APP_VERSION mismatch");
...
assert(worker.includes('const APP_VERSION = "13.0.18";'),"service-worker version mismatch");
assert(worker.includes(`const CACHE_VERSION = "${version.cacheVersion}";`),"service-worker cache mismatch");
assert(worker.includes('asset("./security-profiles.js?v=13.0.18")')&&worker.includes('asset("./security-profiles.css?v=13.0.18")'),"profile assets missing from PWA shell");
...
assert(packageJson.version==="13.0.18"&&packageLock.version==="13.0.18","package version mismatch");
...
assert(worker.includes('asset("./privacy-lock.js?v=13.0.18")'),"privacy module missing from service-worker shell");
assert(workflow.includes("privacy-lock.js"),"GitHub Pages workflow does not deploy privacy module");
assert(exists("SIGNED_OUT_PRIVACY_LOCK_VALIDATION_V13_0_18.md"),"V13.0.18 privacy validation report missing");
...
assert(readme.startsWith("# My Finance Records · V13.0.18 PWA"),"README V13.0.15 heading mismatch");
...
assert(version.cacheVersion.includes("v13018-revision-deadline-r3"),"V13.0.18 revision deadline cache revision mismatch");
...
]) assert(html.includes(token),`V13.0.18 auth UI safeguard missing: ${token}`);
for(const token of [
...
]) assert(cloud.includes(token),`V13.0.18 auth behavior safeguard missing: ${token}`);
assert(readme.includes('Forgot password?')&&readme.includes('Test cloud connection'),"README V13.0.18 auth recovery notes missing");
for(const token of ['id="cloudRecoveryHelpCard"','id="cloudRecoveryEmail"','id="cloudRecoveryCode"','id="cloudRecoveryResend"','id="cloudVerifyRecoveryCode"','id="cloudRecoveryBackToSignIn"']) assert(html.includes(token),`V13.0.18 recovery redirect UI safeguard missing: ${token}`);
for(const token of ['parsePasswordRecoveryUrl','recoveryErrorMessage','verifyRecoveryCode','verifyOtp({ email:value, token:code, type:"recovery" })','url.searchParams.set("auth", "recovery")','Password reset needs attention']) assert(cloud.includes(token),`V13.0.18 recovery redirect behavior safeguard missing: ${token}`);
assert(readme.includes('?auth=recovery')&&readme.includes('{{ .Token }}'),"README V13.0.18 recovery redirect/code instructions missing");
...
]) assert(html.includes(token),`V13.0.18 phone input safeguard missing: ${token}`);
assert(!/<meta[^>]+name=["']viewport["'][^>]+user-scalable\s*=\s*no/i.test(html),"V13.0.18 must not disable user pinch zoom");
assert(!/<meta[^>]+name=["']viewport["'][^>]+maximum-scale\s*=\s*1/i.test(html),"V13.0.18 must not lock maximum zoom scale");
assert(exists("IPHONE_INPUT_ZOOM_VALIDATION_V13_0_17.md"),"V13.0.18 input zoom validation report missing");

if(failures.length){
  console.error("V13.0.18 Signed-Out Privacy Lock validation failed:\n"+failures.map(item=>`- ${item}`).join("\n"));
  process.exit(1);
}
console.log("V13.0.18 Signed-Out Privacy Lock validation passed.");
