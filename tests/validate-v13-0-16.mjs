...
assert(version.version==="13.0.16","version.json is not V13.0.16");
...
assert(html.includes('<title>My Finance Records · V13.0.16</title>'),"HTML title mismatch");
assert(html.includes('const APP_VERSION = "13.0.16";'),"HTML APP_VERSION mismatch");
...
assert(worker.includes('const APP_VERSION = "13.0.16";'),"service-worker version mismatch");
assert(worker.includes(`const CACHE_VERSION = "${version.cacheVersion}";`),"service-worker cache mismatch");
assert(worker.includes('asset("./security-profiles.js?v=13.0.16")')&&worker.includes('asset("./security-profiles.css?v=13.0.16")'),"profile assets missing from PWA shell");
...
assert(packageJson.version==="13.0.16"&&packageLock.version==="13.0.16","package version mismatch");
assert(packageJson.scripts?.quality==="node tests/validate-v13-0-16.mjs","quality script mismatch");
...
assert(readme.includes("My_Finance_Records_V13_0_16_File_Inspection_and_Fixes_Installer.zip")&&readme.includes("Install_V13_0_16.command"),"README V13.0.15 installer filenames are stale");
...
assert(readme.startsWith("# My Finance Records · V13.0.16 PWA"),"README V13.0.15 heading mismatch");
...
assert(version.cacheVersion.includes("v13016-recovery-redirect-r1"),"V13.0.15 cache revision mismatch");
...
assert(html.includes('id="settingsOverviewAppStatus">Version 13.0.16<'),"Settings overview version was not updated");
...
assert(version.name==="Password Recovery Redirect Fix", "V13.0.15 version name mismatch");
...
assert(version.cacheVersion.includes("v13016-recovery-redirect-r1"),"V13.0.15 cache revision mismatch");
assert(readme.startsWith("# My Finance Records · V13.0.16 PWA")&&readme.includes("Brave PWA Install Flow"),"V13.0.15 README metadata missing");
assert(changelog.includes("## 13.0.15 · 2026-08-07"),"V13.0.15 changelog missing");
assert(html.includes('?v=13.0.16')&&worker.includes('?v=13.0.16'),"Version-pinned V13.0.15 assets missing");
...
]) assert(html.includes(token),`V13.0.16 auth UI safeguard missing: ${token}`);
for(const token of [
...
]) assert(cloud.includes(token),`V13.0.16 auth behavior safeguard missing: ${token}`);
assert(readme.includes('Forgot password?')&&readme.includes('Test cloud connection'),"README V13.0.16 auth recovery notes missing");
for(const token of ['id="cloudRecoveryHelpCard"','id="cloudRecoveryEmail"','id="cloudRecoveryCode"','id="cloudRecoveryResend"','id="cloudVerifyRecoveryCode"','id="cloudRecoveryBackToSignIn"']) assert(html.includes(token),`V13.0.16 recovery redirect UI safeguard missing: ${token}`);
for(const token of ['parsePasswordRecoveryUrl','recoveryErrorMessage','verifyRecoveryCode','verifyOtp({ email:value, token:code, type:"recovery" })','url.searchParams.set("auth", "recovery")','Password reset needs attention']) assert(cloud.includes(token),`V13.0.16 recovery redirect behavior safeguard missing: ${token}`);
assert(readme.includes('?auth=recovery')&&readme.includes('{{ .Token }}'),"README V13.0.16 recovery redirect/code instructions missing");
assert(changelog.includes('## 13.0.16 · 2026-08-07'),"V13.0.16 changelog entry missing");
assert(version.name==='Password Recovery Redirect Fix',"V13.0.16 version name mismatch");
assert(changelog.includes('## 13.0.15 · 2026-08-07')&&changelog.includes('Password-reset messaging avoids revealing'),"CHANGELOG V13.0.16 auth entry missing");
assert(version.cacheVersion.includes('v13016-recovery-redirect-r1'),"V13.0.16 auth cache revision mismatch");
assert(packageJson.scripts?.quality==='node tests/validate-v13-0-16.mjs',"V13.0.16 quality script mismatch");

if(failures.length){
  console.error("V13.0.16 Password Recovery Redirect Fix validation failed:\n"+failures.map(item=>`- ${item}`).join("\n"));
  process.exit(1);
}
console.log("V13.0.16 Password Recovery Redirect Fix validation passed.");
