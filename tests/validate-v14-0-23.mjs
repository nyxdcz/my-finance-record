...
const mobileCss=read("mobile-v14-0-23.css");
const css=`${read("app.css")}\n${read("dashboard-interactions.css")}\n${mobileCss}`;
const interactions=read("interaction-patterns.js");
...
const cloudLifecycle=read("cloud-sync-lifecycle.js");
...
const installer=read("Install_V14_0_23.command");
...
assert(version.version==="14.0.23","version.json is not V14.0.23");
assert(version.cacheVersion==="finance-v14-20260813-v1423-phone-sync-recovery-r1","V14.0.23 cache identifier mismatch");
...
assert(html.includes('<title>My Finance Records · V14.0.23</title>'),"HTML title mismatch");
assert(html.includes('const APP_VERSION = "14.0.23";'),"HTML APP_VERSION mismatch");
assert(html.includes('const APP_RELEASE_NAME = "Phone UI & Sync Conflict Recovery";'),"HTML release name mismatch");
...
assert(html.includes('{"version":"V14.0.23","title":"Phone UI & Sync Conflict Recovery"'),"in-app V14.0.23 history entry missing");
const mainScriptStart=html.indexOf('<script>\n    "use strict";');
const interactionBinding='const { closeOverflowMenu, setupOverflowMenus, renderDuplicatedMarquee, renderActiveFilterChips, middleTruncateFilename, createDashboardDragController } = window.FinanceInteractionPatterns;';
assert(mainScriptStart>=0&&html.indexOf(interactionBinding)>=mainScriptStart,"overflow helpers are not bound in the main application scope");
assert(html.indexOf(interactionBinding)===html.lastIndexOf(interactionBinding),"overflow helpers are bound in an unrelated or duplicate script scope");
assert(!read("index.html").includes("?v=14.0.15")&&!worker.includes("?v=14.0.15"),"stale V14.0.15 asset pins remain");
assert(interactions.includes("let overflowMenusReady = false")&&interactions.includes("document.readyState === \"loading\"")&&interactions.includes("DOMContentLoaded"),"overflow menu initialization fix missing");
assert(css.includes('.sidebar .nav-button.active .nav-icon,html[data-theme="dark"] .sidebar .nav-button.active .nav-icon { background:transparent;'),"active navigation still applies a second icon background");
...
const lifecycleSandbox={window:null,setTimeout(fn,delay){const timer={fn,delay};lifecycleTimers.push(timer);return timer;},clearTimeout(timer){const index=lifecycleTimers.indexOf(timer);if(index>=0)lifecycleTimers.splice(index,1);}};
lifecycleSandbox.window=lifecycleSandbox;
vm.createContext(lifecycleSandbox);
try{vm.runInContext(cloudLifecycle,lifecycleSandbox,{filename:"cloud-sync-lifecycle.js"});}catch(error){failures.push(`cloud-sync lifecycle controller bootstrap failed: ${error.stack||error}`);}
const lifecycleReasons=[];let lifecycleReconnects=0;
const lifecycleController=lifecycleSandbox.FinanceCloudSyncLifecycle?.create?.({canPoll:()=>true,canRetry:()=>true,pull:async reason=>{lifecycleReasons.push(reason);},reconnect:async()=>{lifecycleReconnects+=1;}});
lifecycleController?.scheduleForegroundPoll();
assert(lifecycleTimers[0]?.delay===300000,"visible-phone polling does not use the approved five-minute interval");
lifecycleTimers.shift()?.fn();await Promise.resolve();await Promise.resolve();
assert(lifecycleReasons.includes("foreground-poll"),"visible-phone polling did not request a cloud pull");
lifecycleController?.clearForegroundPoll();lifecycleTimers.length=0;
lifecycleController?.scheduleRealtimeRecovery("CHANNEL_ERROR");
assert(lifecycleTimers[0]?.delay===1000,"Realtime recovery does not begin with bounded backoff");
await lifecycleTimers.shift()?.fn();
assert(lifecycleReconnects===1&&lifecycleReasons.includes("realtime-recovery"),"Realtime failure did not reconnect and pull cloud changes");
...
assert(html.includes('./projects-calendar-v13.0.20.js?v=14.0.23')&&html.includes('./projects-calendar-v13.0.20.css?v=14.0.23'),"Project Agenda assets are not version-pinned in HTML");
assert(worker.includes('asset("./projects-calendar-v13.0.20.js?v=14.0.23")')&&worker.includes('asset("./projects-calendar-v13.0.20.css?v=14.0.23")'),"Project Agenda assets are not version-pinned in the offline shell");
...
assert(read("index.html").includes('./app.css?v=14.0.23')&&worker.includes('asset("./app.css?v=14.0.23")'),"extracted application CSS is not versioned and precached");
...
assert(worker.includes('asset("./security-profiles.js?v=14.0.23")')&&worker.includes('asset("./security-profiles.css?v=14.0.23")'),"profile assets missing from PWA shell");
...
assert(worker.includes('asset("./privacy-lock.js?v=15.1.0-ui1")'),"privacy module missing from service-worker shell");
...
const recentReadmeVersions=["V14.0.23","V14.0.22","V14.0.21","V14.0.20","V14.0.19"];
...
assert(exists("Install_V14_0_23.command")&&(fs.statSync(path.join(root,"Install_V14_0_23.command")).mode&0o100)!==0,"V14.0.23 installer is missing or not executable");
assert((fs.statSync(path.join(root,"run_audit.sh")).mode&0o100)!==0,"run_audit.sh is not executable");
assert(installer.includes("npm ci --ignore-scripts")&&installer.includes("npm run quality")&&installer.includes("git diff --check"),"V14.0.23 installer validation flow is incomplete");
...
assert(readme.startsWith("# My Finance Records · V14.0.23"),"README release heading mismatch");
...
const cloudSandbox={
  ...
  data:structuredClone(sampleData),APP_VERSION:"14.0.23",normalizeData:value=>value,saveData(){return true;},renderAll(){},showToast(message,tone){cloudSandbox.__lastToast={message,tone};},STORAGE_KEY:"finance",setInterval(){return 0;},clearInterval(){},setTimeout(){return 0;},clearTimeout(){}
};
...
try{ vm.runInContext(cloudLifecycle,cloudSandbox,{filename:"cloud-sync-lifecycle.js"}); }catch(error){ failures.push(`cloud-sync lifecycle VM bootstrap failed: ${error.stack||error}`); }
...
if(cloudInternals){
  ...
  const normalSetItem=cloudSandbox.localStorage.setItem;
  const constrainedWrites=[];
  let baseWriteFailed=false;
  cloudSandbox.localStorage.setItem=(key,value)=>{constrainedWrites.push(key);if(key.includes("record-base")&&!baseWriteFailed){baseWriteFailed=true;throw new Error("quota");}normalSetItem(key,value);};
  const cloudResolution=cloudInternals.resolveConflict(cloudChoiceKey,"cloud");
  cloudSandbox.localStorage.setItem=normalSetItem;
  ...
  assert(baseWriteFailed&&constrainedWrites[0]?.includes("record-queue")&&constrainedWrites[1]?.includes("record-conflicts"),"conflict resolution did not reclaim queue storage before retrying a constrained baseline save");
  ...
  const directPersistData=structuredClone(sampleData);
  directPersistData.expenses.push({id:"expense-direct",name:"Phone sync fixture",amount:75,paid:false,date:"2026-08-13"});
  cloudInternals.handlePersistedData({detail:{action:"Expense duplicated",data:directPersistData}});
  const directQueue=JSON.parse(cloudMemory.get("simple-finance-cloud-record-queue-v3:profile-personal"));
  const directKey=`expenses\u001fexpense-direct`;
  assert(directQueue[directKey]?.payload?.name==="Phone sync fixture"&&directQueue[directKey]?.reason==="Expense duplicated","direct Finance persistence did not enter the encrypted cloud queue");
  const queuedAfterDirectPersist=cloudSandbox.FinanceCloudSync.status.pendingCount;
  cloudInternals.handlePersistedData({detail:{action:"Duplicate persistence event",data:directPersistData}});
  assert(cloudSandbox.FinanceCloudSync.status.pendingCount===queuedAfterDirectPersist,"repeated persistence event created a duplicate queue item");
  const phoneData={...structuredClone(sampleData),accounts:{Wallet:1900,UnionBank:8157,RCBC:0,Maya:5000,GCash:0,Metrobank:0},accountTypes:{Wallet:"Cash",UnionBank:"Bank",RCBC:"Bank",Maya:"E-wallet",GCash:"E-wallet",Metrobank:"Bank"},accountOrder:["Wallet","UnionBank","RCBC","Maya","GCash","Metrobank"]};
  const desktopData={...structuredClone(sampleData),accounts:{Wallet:300,UnionBank:3520,RCBC:7500,Maya:5000,GCash:0,GoTyme:891},accountTypes:{Wallet:"Cash",UnionBank:"Bank",RCBC:"Bank",Maya:"E-wallet",GCash:"E-wallet",GoTyme:"Bank"},accountOrder:["Wallet","UnionBank","RCBC","Maya","GCash","GoTyme"]};
  const phoneStore=Object.fromEntries(Object.entries(cloudInternals.toRecordMap(phoneData)).map(([key,row])=>[key,{...row,revision:9,deletedAt:"",updatedAt:"2026-08-13T09:00:00Z"}]));
  cloudInternals.seedBaseFromSnapshot(Object.values(phoneStore));
  cloudSandbox.data=structuredClone(desktopData);
  const recoveredKeys=cloudInternals.reconcileUnqueuedLocalChanges("Recovered pre-V14.0.19 Finance state");
  const repairedQueue=JSON.parse(cloudMemory.get("simple-finance-cloud-record-queue-v3:profile-personal"));
  for(const name of ["Wallet","UnionBank","RCBC","Metrobank","GoTyme"]) assert(recoveredKeys.includes(`accounts\u001f${name}`),`existing ${name} account drift was not detected before cloud pull`);
  assert(repairedQueue[`accounts\u001fWallet`]?.payload?.balance===300&&repairedQueue[`accounts\u001fWallet`]?.baseRevision===9,"Wallet repair did not preserve the confirmed cloud revision");
  assert(repairedQueue[`accounts\u001fUnionBank`]?.payload?.balance===3520&&repairedQueue[`accounts\u001fRCBC`]?.payload?.balance===7500,"desktop balance repairs were not queued");
  assert(repairedQueue[`accounts\u001fMetrobank`]?.deleted===true&&repairedQueue[`accounts\u001fGoTyme`]?.payload?.balance===891,"account removal/addition repair was not queued");
  const repairedStore=structuredClone(phoneStore);
  for(const [key,item] of Object.entries(repairedQueue)){if(item.collection!=="accounts")continue;repairedStore[key]={collection:item.collection,recordId:item.recordId,payload:structuredClone(item.payload),sortIndex:item.sortIndex,revision:10,deletedAt:item.deleted?"2026-08-13T10:00:00Z":""};}
  const repairedPhone=cloudInternals.fromRecordStore(repairedStore,phoneData);
  assert(repairedPhone.accounts?.Wallet===300&&repairedPhone.accounts?.UnionBank===3520&&repairedPhone.accounts?.RCBC===7500,"phone reconstruction did not receive desktop balances");
  assert(!Object.prototype.hasOwnProperty.call(repairedPhone.accounts||{},"Metrobank")&&repairedPhone.accounts?.GoTyme===891,"phone reconstruction did not receive the desktop account list");
  const queueCountAfterRepair=cloudSandbox.FinanceCloudSync.status.pendingCount;
  assert(cloudInternals.reconcileUnqueuedLocalChanges().length===0&&cloudSandbox.FinanceCloudSync.status.pendingCount===queueCountAfterRepair,"an already reconciled device created duplicate repair entries");
...
for(const token of [
  'window.addEventListener("finance:data-persisted",handlePersistedData)',
  'window.addEventListener("pageshow"', 'window.addEventListener("pagehide",clearForegroundPoll)',
  'requestLifecycleSync("online",120)', 'requestLifecycleSync("visible",80)',
  '["CHANNEL_ERROR","TIMED_OUT","CLOSED"].includes(normalized)', 'scheduleRealtimeRecovery(normalized)',
  'document.getElementById("cloudOverviewSyncNow")?.addEventListener'
]) assert(cloud.includes(token),`V14.0.23 cross-device sync safeguard missing: ${token}`);
for(const token of ["function reconcileUnqueuedLocalChanges", "!profileCanWrite()", "Recovered unqueued local Finance changes", "const recovered=reconcileUnqueuedLocalChanges()", "previously missed Finance record"]) assert(cloud.includes(token),`V14.0.23 existing-state repair safeguard missing: ${token}`);
assert(cloud.indexOf("const recovered=reconcileUnqueuedLocalChanges()")<cloud.indexOf("await pullChanges();",cloud.indexOf("async function syncNow")),"existing local Finance drift is not queued before the cloud pull");
for(const token of ['const FOREGROUND_POLL_MS = 5 * 60 * 1000','pull("foreground-poll")','pull("realtime-recovery")','function scheduleRealtimeRecovery','function noteRealtimeSubscribed']) assert(cloudLifecycle.includes(token),`V14.0.23 lifecycle controller safeguard missing: ${token}`);
assert(html.includes('./cloud-sync-lifecycle.js?v=14.0.23')&&worker.includes('asset("./cloud-sync-lifecycle.js?v=14.0.23")')&&workflow.includes("cloud-sync-lifecycle.js"),"V14.0.23 lifecycle module is not loaded, cached, and deployed");
assert(changelog.includes("every shared Finance persistence event")&&changelog.includes("30-second visible-device polling fallback"),"V14.0.23 sync changelog scope is incomplete");
assert(changelog.includes("pre-pull reconciliation pass")&&changelog.includes("Metrobank-to-GoTyme"),"V14.0.23 existing-state repair changelog scope is incomplete");
...
for(const token of [
  ...
  "grid-template-areas:\"title sync add tools\" \"month month month month\"",
...
assert(interactions.includes("setupOverflowMenus")&&html.includes("setupOverflowMenus()"),"accessible reusable overflow-menu setup is missing");
...
for(const token of [
  ...
  'id="settings-panel-overview"','settingsOverviewAccountsChip','settingsOverviewCalendarChip','settingsOverviewProfileChip','settingsOverviewSyncChip','settingsOverviewAppChip',
  'settings-mobile-back','Back to Settings','SETTINGS_PANEL_ALIASES','cloud:"sync"','backup:"sync"','offline:"app"','advanced:"app"',
  'settings-nav-group','Start','Your data','Protection','settings-status-copy','settings-danger-zone',
  'simplifyAccountLedgerSettings','data-settings-ledger-disclosure','simplifyReminderSettingsCard','data-settings-more-options="app"',
  'addSettingsWalkthroughActions','settings-walkthrough','Back to overview','Next: ${SETTINGS_PANEL_LABELS[nextKey]}','setupSettingsFormDirtyState','updateSettingsOverview'
]) assert(html.includes(token),`V13.0.2 simplified Settings safeguard missing: ${token}`);
assert((html.match(/settingsDisclosure\("More options"/g)||[]).length>=4,"Settings sections do not consistently consolidate secondary controls");
...
assert(version.cacheVersion.includes("v1423-phone-sync-recovery-r1"),"V14.0.23 cache revision mismatch");
...
assert(changelog.includes("numbered project revision cycles"),"V13.0.9 project revision documentation incomplete");
...
assert(html.includes('id="settingsOverviewAppStatus">Version 14.0.23<'),"Settings overview version was not updated");
...
assert(changelog.includes("## 13.0.9 · 2026-08-07"),"V13.0.9 release history missing from CHANGELOG");
...
assert(changelog.includes("Completed Projects whether fully paid or still carrying a client balance"),"V13.0.9 documentation incomplete");
...
assert(changelog.includes("## 13.0.9 · 2026-08-07")&&changelog.includes("## 13.0.10 · 2026-08-07"),"V13.0.9 documentation incomplete");
...
assert(version.name==="Phone UI & Sync Conflict Recovery", "V14.0.23 version name mismatch");
...
assert(changelog.includes("## 13.0.12 · 2026-08-07"),"V13.0.15 documentation missing");
...
assert(version.cacheVersion.includes("v1423-phone-sync-recovery-r1"),"V14.0.23 cache revision mismatch");
assert(readme.startsWith("# My Finance Records · V14.0.23"),"V14.0.23 README metadata missing");
assert(changelog.includes("## 13.0.15 · 2026-08-07"),"V13.0.15 changelog missing");
assert(html.includes('?v=14.0.23')&&worker.includes('?v=14.0.23'),"Version-pinned V13.0.15 assets missing");
...
assert(version.name==="Phone UI & Sync Conflict Recovery","V14.0.23 version name mismatch");
for(const token of ["#paid-expenses [data-paid-expense-row]","grid-template-columns: minmax(0, 1fr) 44px",".cloud-device-table thead",".cloud-conflict-review-footer","data-label=\"Cloud version\""]) assert(mobileCss.includes(token)||conflictReview.includes(token),`V14.0.23 phone layout safeguard missing: ${token}`);
for(const token of ["function persist({ reclaimFirst = false } = {})","reclaimFirst && write()","persist:()=>persist({reclaimFirst:true})"]) assert(cloud.includes(token),`V14.0.23 conflict storage recovery safeguard missing: ${token}`);
assert(html.includes('./mobile-v14-0-23.css?v=14.0.23')&&worker.includes('asset("./mobile-v14-0-23.css?v=14.0.23")')&&workflow.includes("mobile-v14-0-23.css"),"V14.0.23 phone stylesheet is not loaded, cached, and deployed");
assert(version.cacheVersion.includes("v1423-phone-sync-recovery-r1"),"V14.0.23 cache revision mismatch");
...
]) assert(html.includes(token),`V14.0.23 auth UI safeguard missing: ${token}`);
for(const token of [
...
]) assert(cloud.includes(token),`V14.0.23 auth behavior safeguard missing: ${token}`);
assert(changelog.includes('Forgot password')&&changelog.includes('Test cloud connection'),"CHANGELOG auth recovery notes missing");
for(const token of ['id="cloudRecoveryHelpCard"','id="cloudRecoveryEmail"','id="cloudRecoveryCode"','id="cloudRecoveryResend"','id="cloudVerifyRecoveryCode"','id="cloudRecoveryBackToSignIn"']) assert(html.includes(token),`V14.0.23 recovery redirect UI safeguard missing: ${token}`);
for(const token of ['parsePasswordRecoveryUrl','recoveryErrorMessage','verifyRecoveryCode','verifyOtp({ email:value, token:code, type:"recovery" })','url.searchParams.set("auth", "recovery")','Password reset needs attention']) assert(cloud.includes(token),`V14.0.23 recovery redirect behavior safeguard missing: ${token}`);
...
assert(changelog.includes('## 14.0.23 · 2026-08-13'),"V14.0.23 changelog entry missing");
assert(version.name==='Phone UI & Sync Conflict Recovery',"V14.0.23 version name mismatch");
assert(changelog.includes('## 13.0.15 · 2026-08-07')&&changelog.includes('Password-reset messaging avoids revealing'),"CHANGELOG V14.0.23 auth entry missing");
assert(version.cacheVersion.includes('v1423-phone-sync-recovery-r1'),"V14.0.23 auth cache revision mismatch");
assert(packageJson.scripts?.quality==='npm run inspect && npm run lint && npm run maintainability && npm run test',"V14.0.23 quality script mismatch");


// V14.0.23 iPhone input zoom prevention safeguards.
assert(version.name==="Phone UI & Sync Conflict Recovery","V14.0.23 version name mismatch");
assert(version.cacheVersion.includes("v1423-phone-sync-recovery-r1"),"V14.0.23 cache revision mismatch");
assert(read("IPHONE_INPUT_ZOOM_VALIDATION_V13_0_17.md").includes("16px"),"V14.0.23 input-zoom guidance missing");
...
]) assert(html.includes(token),`V14.0.23 phone input safeguard missing: ${token}`);
assert(!/<meta[^>]+name=["']viewport["'][^>]+user-scalable\s*=\s*no/i.test(html),"V14.0.23 must not disable user pinch zoom");
assert(!/<meta[^>]+name=["']viewport["'][^>]+maximum-scale\s*=\s*1/i.test(html),"V14.0.23 must not lock maximum zoom scale");
assert(exists("IPHONE_INPUT_ZOOM_VALIDATION_V13_0_17.md"),"V14.0.23 input zoom validation report missing");

// V14.0.23 UI accessibility and signed-out Settings reliability safeguards.
for(const token of [
...
]) assert(html.includes(token),`V14.0.23 visual safeguard missing: ${token}`);
for(const token of [
...
]) assert(html.includes(token),`V14.0.23 interface safeguard missing: ${token}`);
for(const token of [
  "[data-help-key]",
...
]) assert(privacyLock.includes(token),`V14.0.23 privacy safeguard missing: ${token}`);
...
for(const token of [
  ...
  'expense editing keeps overflow helpers available to the live application'
]) assert(browserCoverage.includes(token),`V14.0.23 browser coverage missing: ${token}`);
assert(readme.includes("**V14.0.23 · Phone UI & Sync Conflict Recovery**"),"README V14.0.23 release notes missing");
for(const token of [
...
]) assert(html.includes(token),`V14.0.23 history or Settings search safeguard missing: ${token}`);
for(const token of ["baseRedoLastChange", 'key === "y"', 'key === "z" && event.shiftKey']) assert(productivity.includes(token),`V14.0.23 redo shortcut safeguard missing: ${token}`);
for(const token of ['#settingsSearchButton', '#settingsSearchInput', '[data-settings-search-result]']) assert(privacyLock.includes(token),`V14.0.23 signed-out Settings search safeguard missing: ${token}`);

// V14.0.23 approved Dashboard arrangement and temporary compact-rail expansion.
...
assert(Boolean(dashboardOrderSource)&&JSON.parse(dashboardOrderSource).join("|")===expectedDashboardOrder.join("|"),"approved Dashboard default order is missing");
for(const [key,size] of Object.entries(expectedDashboardSizes)) assert(html.includes(`"${key}": "${size}"`),`Dashboard default size mismatch for ${key}`);
for(const [index,key] of expectedDashboardOrder.entries()) assert(html.includes(`.dashboard-default-layout [data-dashboard-card="${key}"] { order:${index+1}; }`),`responsive Dashboard order mismatch for ${key}`);
for(const token of [
  "function defaultDashboardPreferences()",
  "if (!customized) return defaultDashboardPreferences()",
  "dashboardPreferences = defaultDashboardPreferences()",
  "const previewExpandedRail = isDesktopSidebar() && !sidebarPinned",
  "if (previewExpandedRail) openMenu();",
  "max-width:170px",
  "transition:max-width .26s cubic-bezier(.2,.8,.2,1)"
]) assert(html.includes(token),`V14.0.23 Dashboard or rail behavior missing: ${token}`);
...
function evaluateDashboardPreferences(saved){
  ...
  if(saved!==undefined) memory.set("finance-dashboard-phase4",JSON.stringify(saved));
  const sandbox={STORAGE_KEY:"finance",localStorage:{getItem:key=>memory.get(key)??null,setItem:(key,value)=>memory.set(key,String(value))},globalThis:null};
  sandbox.globalThis=sandbox;
  vm.createContext(sandbox);
  vm.runInContext(`${dashboardModule}\nglobalThis.__dashboard={order:[...DEFAULT_DASHBOARD_ORDER],sizes:{...DEFAULT_DASHBOARD_SIZES},preferences:dashboardPreferences};`,sandbox);
  return sandbox.__dashboard;
}
try{
  const fresh=evaluateDashboardPreferences();
  const uncustomized=evaluateDashboardPreferences({customized:false,order:[...expectedDashboardOrder].reverse(),sizes:{"cash-flow":"large"}});
  const customOrder=[...expectedDashboardOrder].reverse();
  const customized=evaluateDashboardPreferences({customized:true,order:customOrder,sizes:{"cash-flow":"large","calendar":"large"}});
  assert(fresh.preferences.order.join("|")===expectedDashboardOrder.join("|")&&JSON.stringify(fresh.preferences.sizes)===JSON.stringify(expectedDashboardSizes),"fresh Dashboard preferences do not use the approved defaults");
...
assert(changelog.includes("approved Dashboard card sequence")&&changelog.includes("saved pin state"),"V14.0.23 changelog scope is incomplete");

// V14.0.23 primary navigation labels and Philippine-time appearance schedule.
for(const [page,label] of Object.entries({dashboard:"Overview",money:"Finance",projects:"Work",reports:"Insights"})) {
  assert(html.includes(`data-page="${page}" data-nav-label="${label}"`),`V14.0.23 sidebar label missing for ${page}`);
}
for(const token of [
  'timeZone:"Asia/Manila"',
  'return minutes >= 421 && minutes < 1080 ? "light" : "dark"',
  'Appearance: Philippine time',
  '>Philippine time</small>',
  'const labels = { system: "Philippine time", light: "Light", dark: "Dark" }',
  'Philippine time · ${resolved === "light" ? "Day" : "Night"}',
  'function schedulePhilippineThemeRefresh()',
  'document.addEventListener("visibilitychange"'
]) assert(html.includes(token),`V14.0.23 Philippine-time theme safeguard missing: ${token}`);
...
try {
  const themeSandbox={window:null,globalThis:null};
  themeSandbox.window=themeSandbox;
  themeSandbox.globalThis=themeSandbox;
  vm.createContext(themeSandbox);
  vm.runInContext(`${themeFunctionSource}\nglobalThis.__themeAt = window.financePhilippineThemeAt;`,themeSandbox);
  const boundaries=[
    ["2026-08-11T23:00:00.000Z","dark","7:00 AM"],
    ["2026-08-11T23:01:00.000Z","light","7:01 AM"],
    ["2026-08-12T09:59:00.000Z","light","5:59 PM"],
    ["2026-08-12T10:00:00.000Z","dark","6:00 PM"]
  ];
  for(const [iso,expected,label] of boundaries) assert(themeSandbox.__themeAt(new Date(iso))===expected,`Philippine ${label} should resolve to ${expected} mode`);
} catch(error) {
  failures.push(`Philippine theme boundary fixture failed: ${error.stack||error}`);
}
assert(changelog.includes("Asia/Manila")&&changelog.includes("7:01 AM")&&changelog.includes("6:00 PM"),"V14.0.23 Manila schedule documentation is incomplete");

// V14.0.23 duplicate sidebar group-heading removal.
...
]) assert(browserCoverage.includes(token),`V14.0.23 responsive sidebar coverage missing: ${token}`);
assert(changelog.includes("duplicated uppercase Overview, Finance, Work, and Insights headings"),"V14.0.23 sidebar cleanup documentation is incomplete");

// V14.0.23 accessible Dashboard interaction system.
assert(!/<marquee\b/i.test(html),"deprecated marquee element must not be used");
for(const token of [
  'id="dashboardWeekMarquee"', 'id="dashboardWeekMarqueeTrack"',
  "function renderDashboardWeekMarquee()", "Array.from({ length:7 }", 'renderDuplicatedMarquee(track, days.join(""))'
]) assert(html.includes(token),`one-week marquee safeguard missing: ${token}`);
assert(interactions.includes('track.innerHTML = `<div class="dashboard-week-marquee-group">${groupContent}</div><div class="dashboard-week-marquee-group" aria-hidden="true">${groupContent}</div>`'),"exact duplicated marquee content is not implemented");
for(const token of [
  "@keyframes dashboard-week-marquee", "transform:translateX(-50%)", "animation-play-state:paused", "prefers-reduced-motion:reduce",
  "overflow:hidden; white-space:nowrap; text-overflow:ellipsis", "-webkit-box-orient:vertical", "-webkit-line-clamp:2", "mask-image:linear-gradient"
]) assert(css.includes(token),`marquee or truncation style missing: ${token}`);
for(const token of ["function middleTruncateFilename", "lastIndexOf"]) assert(interactions.includes(token),`extension-preserving filename truncation missing: ${token}`);
assert(html.includes("filename-middle-truncated"),"filename truncation output is missing");

assert(html.includes('id="topbarToolsTrigger"')&&html.includes('aria-haspopup="menu" aria-controls="topbarToolsPanel" aria-expanded="false"'),"topbar meatballs button semantics missing");
assert(!html.includes('<details class="topbar-tools-menu')&&!html.includes('<summary aria-label="More tools"'),"legacy topbar disclosure remains");
for(const token of ["kebab-icon", 'role="menu"', 'role="menuitem"', 'role="separator"']) assert(html.includes(token),`overflow menu markup missing: ${token}`);
for(const token of ["function openOverflowMenu", "function closeOverflowMenu", 'event.key === "Escape"']) assert(interactions.includes(token),`overflow menu behavior missing: ${token}`);
assert(html.includes('id="menuButton" type="button" aria-label="Show navigation" aria-controls="sidebar" aria-expanded="false"'),"navigation hamburger semantics changed incorrectly");

for(const token of ["data-dashboard-drag", "draggable = true", "dashboard-drag-preview", "dashboard-drop-before", "application/x-dashboard-card", "Available destination"]) assert(interactions.includes(token)||css.includes(token),`Dashboard drag-and-drop safeguard missing: ${token}`);
for(const token of ["dashboardDragAnnouncer", "Dashboard order saved"]) assert(html.includes(token),`Dashboard drag result safeguard missing: ${token}`);
assert(!html.includes("aria-grabbed"),"deprecated aria-grabbed state must not be used");
assert(html.includes('button.className = "dashboard-resize-handle"'),"Dashboard resize selection handle missing");

assert(html.includes('<progress class="progress-track" id="dashPaymentProgress" max="100" value="0"')&&html.includes('<progress class="savings-goal-progress" max="100" value="${progressValue}"'),"native determinate progress elements missing");
assert(html.includes('id="migrationSpinner" role="status"')&&cloud.includes('top.setAttribute("aria-busy", syncing ? "true" : "false")'),"indeterminate migration or cloud-sync indicator missing");
assert(css.includes("border-block-start")&&css.includes("border-block-end"),"decorative rules do not use logical CSS borders");
assert(changelog.includes("one-week Dashboard calendar marquee")&&changelog.includes("ARIA live announcements")&&changelog.includes('aria-haspopup="menu"'),"V14.0.23 changelog scope is incomplete");

// V14.0.23 Finance marquee, native focus ring, and compact component roles.
for(const token of ['id="financeWeekMarquee"','id="financeWeekMarqueeTrack"','id="financeWeekMarqueeRange"','function renderFinanceWeekMarquee()','renderFinanceWeekMarquee();']) assert(html.includes(token),`Finance marquee safeguard missing: ${token}`);
assert(html.includes('function renderWeekMarquee(trackId, rangeId, anchorText)')&&html.includes('renderWeekMarquee("dashboardWeekMarqueeTrack"')&&html.includes('["financeWeekMarqueeTrack","financeWeekMarqueeRange"]'),"Dashboard and Finance do not share the marquee renderer");
for(const token of ['.app .main :where(a[href],button,input,select,textarea,summary','focus-visible { outline:auto!important; outline-offset:3px!important; }','.ui-badge {','.ui-chip {','.ui-chip-remove {','.ui-pill,.status-pill {','.ui-tag {']) assert(css.includes(token),`native focus or compact component style missing: ${token}`);
for(const token of ['id="expenseActiveFilterChips"','class="status-badge ui-pill status-unpaid"','class="status-badge ui-pill status-paid"','class="ui-tag"','badge.className = "mobile-filter-count ui-badge"']) assert(html.includes(token),`compact component markup missing: ${token}`);
for(const token of ['function renderActiveFilterChips','class="ui-chip"','class="ui-chip-remove"','aria-label="Remove ${String(label)','data-remove-filter']) assert(interactions.includes(token),`dismissible filter Chip behavior missing: ${token}`);
for(const token of ['#financeWeekMarqueeTrack > .dashboard-week-marquee-group','Remove Category: Subscriptions filter','getComputedStyle(node).outlineStyle','.record-row .ui-tag','.record-row .ui-pill']) assert(browserCoverage.includes(token),`V14.0.23 browser coverage missing: ${token}`);
assert(changelog.includes("native browser focus rings")&&changelog.includes("active-filter Chips")&&changelog.includes("category Tags"),"V14.0.23 changelog scope is incomplete");

// V14.0.23 43px marquee row, phone removal, and stable pointer-exit sidebar.
for(const token of ['id="dashboardWeekMarquee"','id="dashboard-title"','id="incomeFinanceWeekMarquee"','id="paidFinanceWeekMarquee"']) assert(html.includes(token),`V14.0.23 marquee placement missing: ${token}`);
assert(html.indexOf('id="dashboardWeekMarquee"')<html.indexOf('id="dashboard-title"'),"Dashboard marquee is not above Monthly overview");
for(const page of ["income","paid-expenses"]) { const start=html.indexOf(`id="${page}"`), next=html.indexOf('class="workspace-switcher money-workspace-switcher',start); assert(start>=0&&html.indexOf('finance-week-marquee',start)<next,`${page} marquee is not above the Finance tabs`); }
const moneyStart=html.indexOf('id="money"'), moneyRow=html.indexOf('class="finance-workspace-marquee-row no-print"',moneyStart), moneyTabs=html.indexOf('class="workspace-switcher money-workspace-switcher"',moneyRow), moneyMarquee=html.indexOf('id="financeWeekMarquee"',moneyRow);
assert(moneyStart>=0&&moneyRow>moneyStart&&moneyTabs>moneyRow&&moneyMarquee>moneyTabs,"Budget & Expenses marquee is not beside and after the Finance tabs in their shared row");
for(const token of ['height:43px; min-height:43px; max-height:43px','finance-workspace-marquee-row','> .money-workspace-switcher { position:static; top:auto; flex:0 0 auto; height:43px; min-height:43px; max-height:43px; box-sizing:border-box','> .finance-week-marquee { flex:1 1 auto','.dashboard-week-marquee { display:none!important; }']) assert(css.includes(token),`V14.0.23 marquee layout safeguard missing: ${token}`);
for(const [page,file] of Object.entries({dashboard:"sidebar-overview.png",money:"sidebar-finance.png",projects:"sidebar-work.png",settings:"sidebar-settings.png"})) assert(html.includes(`data-page="${page}" data-nav-label`)&&html.includes(`src="./icons/${file}"`),`V14.0.23 supplied ${page} icon missing`);
assert(html.includes('class="nav-button insights-nav-button"')&&!html.includes('data-page="reports" data-nav-label="Insights"><span class="nav-icon"'),"Insights icon was not removed");
for(const file of ["sidebar-overview.png","sidebar-finance.png","sidebar-work.png","sidebar-settings.png"]) assert(exists(`icons/${file}`)&&worker.includes(`asset("./icons/${file}")`),`sidebar asset is missing or not precached: ${file}`);
assert(!html.includes('sidebarEdgeTrigger')&&!html.includes('scheduleSidebarExpand')&&!html.includes('sidebarElement.addEventListener("mouseenter"')&&!html.includes('sidebarElement.addEventListener("focusin"'),"sidebar still expands without a click");
assert(html.includes('if (previewExpandedRail) openMenu();')&&html.indexOf('if (previewExpandedRail) openMenu();')<html.indexOf('goToPage(button.dataset.page, { focusHeading: true });'),"sidebar icon click does not expand before navigation");
assert(html.includes('sidebarElement.addEventListener("mouseleave"')&&!html.includes('window.addEventListener("scroll", () => closeMenu')&&!html.includes('scheduleSidebarHide'),"sidebar does not wait for pointer exit before retracting");
assert(html.includes('if (!isDesktopSidebar()) closeMenu(false);'),"desktop page navigation still retracts the sidebar");
for(const token of ['position:absolute; top:10px; left:64px','sidebar.desktop-open .sidebar-navigation,.sidebar.sidebar-pinned .sidebar-navigation { margin-top:48px','left:10px; right:auto; width:44px; height:42px']) assert(css.includes(token),`stable sidebar icon geometry missing: ${token}`);
...
assert(changelog.includes("43px desktop/tablet")&&changelog.includes("pointer leaves")&&changelog.includes("same location"),"V14.0.23 release documentation is incomplete");
...
for(const token of ['collapsed Monthly budget plan is compact and keeps native control semantics','Toast uses a pausable live status and persistent warnings','await toast.hover()','await dismiss.focus()']) assert(browserCoverage.includes(token),`V14.0.23 budget or Toast browser coverage missing: ${token}`);
assert(changelog.includes("collapsed Monthly budget plan")&&changelog.includes("pausable success timing")&&changelog.includes("persistent warnings and errors"),"V14.0.23 compact budget or Toast documentation is incomplete");

if(failures.length){
  console.error("V14.0.23 Phone UI & Sync Conflict Recovery validation failed:\n"+failures.map(item=>`- ${item}`).join("\n"));
  process.exit(1);
}
console.log("V14.0.23 Phone UI & Sync Conflict Recovery validation passed.");
