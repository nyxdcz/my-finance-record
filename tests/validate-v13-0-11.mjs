...
assert(version.version==="13.0.11","version.json is not V13.0.11");
...
assert(html.includes('<title>My Finance Records · V13.0.11</title>'),"HTML title mismatch");
assert(html.includes('const APP_VERSION = "13.0.11";'),"HTML APP_VERSION mismatch");
...
assert(worker.includes('const APP_VERSION = "13.0.11";'),"service-worker version mismatch");
...
assert(packageJson.version==="13.0.11"&&packageLock.version==="13.0.11","package version mismatch");
assert(packageJson.scripts?.quality==="node tests/validate-v13-0-11.mjs","quality script mismatch");
...
assert(readme.includes("My_Finance_Records_V13_0_11_File_Inspection_and_Fixes_Installer.zip")&&readme.includes("Install_V13_0_11.command"),"README V13.0.11 installer filenames are stale");
...
assert(readme.startsWith("# My Finance Records · V13.0.11 PWA"),"README V13.0.11 heading mismatch");
...
assert(changelog.includes("## 13.0.11 · 2026-08-07")&&changelog.includes("## 13.0.10 · 2026-08-07"),"CHANGELOG V13.0.11 entry or V13.0.10 history missing");
...
assert(version.cacheVersion.includes("v13011-ui-interaction-r1"),"V13.0.10 cache revision mismatch");
...
assert(html.includes('id="settingsOverviewAppStatus">Version 13.0.11<'),"Settings overview version was not updated");
...
assert(version.name==="UI & Interaction Reliability", "V13.0.11 version name mismatch");
...
// V13.0.11 interaction reliability and project mobile cleanup safeguards.
for(const token of [
...
]) assert(html.includes(token)||ledgerSource.includes(token),`V13.0.11 interaction/UI safeguard missing: ${token}`);
...
assert(readme.includes("V13.0.11 · UI & Interaction Reliability")&&changelog.includes("## 13.0.11 · 2026-08-07"),"V13.0.11 documentation missing");
assert(exists("UI_INTERACTION_RELIABILITY_VALIDATION_V13_0_11.md"),"V13.0.11 validation report missing");

if(failures.length){
  console.error("V13.0.11 UI & Interaction Reliability validation failed:\n"+failures.map(item=>`- ${item}`).join("\n"));
  process.exit(1);
}
console.log("V13.0.11 UI & Interaction Reliability validation passed.");
