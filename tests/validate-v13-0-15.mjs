...
assert(version.version==="13.0.15","version.json is not V13.0.15");
...
assert(html.includes('<title>My Finance Records · V13.0.15</title>'),"HTML title mismatch");
assert(html.includes('const APP_VERSION = "13.0.15";'),"HTML APP_VERSION mismatch");
...
assert(worker.includes('const APP_VERSION = "13.0.15";'),"service-worker version mismatch");
assert(worker.includes(`const CACHE_VERSION = "${version.cacheVersion}";`),"service-worker cache mismatch");
assert(worker.includes('asset("./security-profiles.js?v=13.0.15")')&&worker.includes('asset("./security-profiles.css?v=13.0.15")'),"profile assets missing from PWA shell");
...
assert(packageJson.version==="13.0.15"&&packageLock.version==="13.0.15","package version mismatch");
assert(packageJson.scripts?.quality==="node tests/validate-v13-0-15.mjs","quality script mismatch");
...
assert(readme.includes("My_Finance_Records_V13_0_15_File_Inspection_and_Fixes_Installer.zip")&&readme.includes("Install_V13_0_15.command"),"README V13.0.15 installer filenames are stale");
...
assert(read("CLOUD_SYNC_SETUP.md").includes("Password reset redirect (V13.0.15+)"),"V13.0.15 password-reset redirect setup guidance missing");
assert(readme.startsWith("# My Finance Records · V13.0.15 PWA"),"README V13.0.15 heading mismatch");
...
const cloudSandbox={
  console,structuredClone,crypto:crypto.webcrypto,URL,window:null,globalThis:null,FinanceProfileArchitecture:cloudArchitecture,
  ...
  navigator:{onLine:true,userAgent:"Node",platform:"MacIntel"},document:{readyState:"loading",addEventListener(){},getElementById(){return null;}},location:{href:"https://app.test/index.html?recovery=1#cloud",reload(){}},matchMedia(){return{matches:false}},
  ...
  __authFixture:{reset:null,updatedPassword:""},
  financeLoadSupabase:async()=>({createClient:()=>({auth:{onAuthStateChange(){return{data:{subscription:{unsubscribe(){}}}}},async resetPasswordForEmail(email,options){cloudSandbox.__authFixture.reset={email,options};return{data:{},error:null}},async updateUser({password}){cloudSandbox.__authFixture.updatedPassword=password;return{data:{user:{id:"fixture-user",email:"fixture@example.com"}},error:null}}}})}),
...
if(cloudInternals){
  ...
  assert(cloudInternals.passwordRecoveryRedirect()==="https://app.test/index.html","password recovery redirect did not strip query/hash");
  await cloudInternals.requestPasswordReset("fixture@example.com");
  assert(cloudSandbox.__authFixture.reset?.email==="fixture@example.com"&&cloudSandbox.__authFixture.reset?.options?.redirectTo==="https://app.test/index.html","password-reset request did not use the secure app redirect");
...
assert(version.cacheVersion.includes("v13015-auth-recovery-r1"),"V13.0.15 cache revision mismatch");
...
assert(html.includes('id="settingsOverviewAppStatus">Version 13.0.15<'),"Settings overview version was not updated");
...
assert(version.name==="Cloud Sign-in Recovery & Diagnostics", "V13.0.15 version name mismatch");
...
assert(version.cacheVersion.includes("v13015-auth-recovery-r1"),"V13.0.15 cache revision mismatch");
assert(readme.startsWith("# My Finance Records · V13.0.15 PWA")&&readme.includes("Brave PWA Install Flow"),"V13.0.15 README metadata missing");
assert(changelog.includes("## 13.0.15 · 2026-08-07"),"V13.0.15 changelog missing");
assert(html.includes('?v=13.0.15')&&worker.includes('?v=13.0.15'),"Version-pinned V13.0.15 assets missing");
...
]) assert(html.includes(token),`V13.0.15 auth UI safeguard missing: ${token}`);
for(const token of [
...
]) assert(cloud.includes(token),`V13.0.15 auth behavior safeguard missing: ${token}`);
assert(readme.includes('Forgot password?')&&readme.includes('Test cloud connection'),"README V13.0.15 auth recovery notes missing");
assert(changelog.includes('## 13.0.15 · 2026-08-07')&&changelog.includes('Password-reset messaging avoids revealing'),"CHANGELOG V13.0.15 auth entry missing");
assert(version.cacheVersion.includes('v13015-auth-recovery-r1'),"V13.0.15 auth cache revision mismatch");
assert(packageJson.scripts?.quality==='node tests/validate-v13-0-15.mjs',"V13.0.15 quality script mismatch");

if(failures.length){
  console.error("V13.0.15 Cloud Sign-in Recovery & Diagnostics validation failed:\n"+failures.map(item=>`- ${item}`).join("\n"));
  process.exit(1);
}
console.log("V13.0.15 Cloud Sign-in Recovery & Diagnostics validation passed.");
