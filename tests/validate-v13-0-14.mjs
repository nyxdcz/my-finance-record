...
assert(version.version==="13.0.14","version.json is not V13.0.14");
...
assert(html.includes('<title>My Finance Records · V13.0.14</title>'),"HTML title mismatch");
assert(html.includes('const APP_VERSION = "13.0.14";'),"HTML APP_VERSION mismatch");
...
assert(worker.includes('const APP_VERSION = "13.0.14";'),"service-worker version mismatch");
assert(worker.includes(`const CACHE_VERSION = "${version.cacheVersion}";`),"service-worker cache mismatch");
assert(worker.includes('asset("./security-profiles.js?v=13.0.14")')&&worker.includes('asset("./security-profiles.css?v=13.0.14")'),"profile assets missing from PWA shell");
...
assert(packageJson.version==="13.0.14"&&packageLock.version==="13.0.14","package version mismatch");
assert(packageJson.scripts?.quality==="node tests/validate-v13-0-14.mjs","quality script mismatch");
...
assert(readme.includes("My_Finance_Records_V13_0_14_File_Inspection_and_Fixes_Installer.zip")&&readme.includes("Install_V13_0_14.command"),"README V13.0.14 installer filenames are stale");
...
assert(readme.startsWith("# My Finance Records · V13.0.14 PWA"),"README V13.0.14 heading mismatch");
...
assert(changelog.includes("## 13.0.12 · 2026-08-07")&&changelog.includes("## 13.0.10 · 2026-08-07"),"CHANGELOG V13.0.14 entry or V13.0.10 history missing");
...
assert(version.cacheVersion.includes("v13014-brave-install-r1"),"V13.0.14 cache revision mismatch");
...
assert(html.includes('id="settingsOverviewAppStatus">Version 13.0.14<'),"Settings overview version was not updated");
...
assert(version.name==="Brave PWA Install Flow", "V13.0.14 version name mismatch");
...
// V13.0.14 interaction reliability and project mobile cleanup safeguards.
for(const token of [
...
]) assert(html.includes(token)||ledgerSource.includes(token),`V13.0.14 interaction/UI safeguard missing: ${token}`);
...
assert(readme.includes("V13.0.12 · Spend Reliability & Phone Budget Compaction")&&changelog.includes("## 13.0.12 · 2026-08-07"),"V13.0.14 documentation missing");
assert(exists("UI_INTERACTION_RELIABILITY_VALIDATION_V13_0_11.md"),"V13.0.11 validation report missing");
assert(exists("RECORD_SPENDING_TRANSACTION_VALIDATION_V13_0_13.md"),"V13.0.14 transaction validation report missing");


// V13.0.14 Record Spending transaction hotfix safeguards.
...
]) assert(ledgerV1313.includes(token),`V13.0.14 transaction safeguard missing: ${token}`);
...
assert(version.cacheVersion.includes("v13014-brave-install-r1"),"V13.0.14 cache revision mismatch");
assert(readme.startsWith("# My Finance Records · V13.0.14 PWA")&&readme.includes("Brave PWA Install Flow"),"V13.0.14 README metadata missing");
assert(changelog.includes("## 13.0.14 · 2026-08-07"),"V13.0.14 changelog missing");
assert(html.includes('?v=13.0.14')&&worker.includes('?v=13.0.14'),"Version-pinned V13.0.14 assets missing");
assert(ledgerV1313.includes('const stored = JSON.parse(storedRaw);'),"V13.0.14 storage verification must inspect the raw persisted ledger");
...
// V13.0.14 Brave PWA installation safeguards.
for (const token of [
...
]) assert(html.includes(token),`V13.0.14 Brave install safeguard missing: ${token}`);
...
assert(changelog.includes("## 13.0.14 · 2026-08-07"),"V13.0.14 changelog entry missing");

if(failures.length){
  console.error("V13.0.14 Brave PWA Install Flow validation failed:\n"+failures.map(item=>`- ${item}`).join("\n"));
  process.exit(1);
}
console.log("V13.0.14 Brave PWA Install Flow validation passed.");
