...
assert(installer.includes("npm ci --ignore-scripts")&&installer.includes("npm run quality")&&installer.includes("git diff --check"),"V14.0.12 installer validation flow is incomplete");
...
assert(readme.startsWith("# My Finance Records · V14.0.12"),"README release heading mismatch");
...
const cloudSandbox={
  ...
  data:structuredClone(sampleData),APP_VERSION:"14.0.12",normalizeData:value=>value,saveData(){return true;},renderAll(){},showToast(message,tone){cloudSandbox.__lastToast={message,tone};},STORAGE_KEY:"finance",setInterval(){return 0;},clearInterval(){},setTimeout(){return 0;},clearTimeout(){}
};
...
assert(version.cacheVersion.includes("v1412-dashboard-rail-r1"),"V13.0.15 cache revision mismatch");
...
assert(html.includes('id="settingsOverviewAppStatus">Version 14.0.12<'),"Settings overview version was not updated");
...
assert(version.name==="Dashboard Defaults & Rail Motion", "V14.0.12 version name mismatch");
...
assert(version.cacheVersion.includes("v1412-dashboard-rail-r1"),"V13.0.15 cache revision mismatch");
assert(readme.startsWith("# My Finance Records · V14.0.12"),"V14.0.12 README metadata missing");
assert(changelog.includes("## 13.0.15 · 2026-08-07"),"V13.0.15 changelog missing");
assert(html.includes('?v=14.0.12')&&worker.includes('?v=14.0.12'),"Version-pinned V13.0.15 assets missing");
...
]) assert(html.includes(token),`V14.0.12 auth UI safeguard missing: ${token}`);
for(const token of [
...
]) assert(cloud.includes(token),`V14.0.12 auth behavior safeguard missing: ${token}`);
assert(changelog.includes('Forgot password')&&changelog.includes('Test cloud connection'),"CHANGELOG auth recovery notes missing");
for(const token of ['id="cloudRecoveryHelpCard"','id="cloudRecoveryEmail"','id="cloudRecoveryCode"','id="cloudRecoveryResend"','id="cloudVerifyRecoveryCode"','id="cloudRecoveryBackToSignIn"']) assert(html.includes(token),`V14.0.12 recovery redirect UI safeguard missing: ${token}`);
for(const token of ['parsePasswordRecoveryUrl','recoveryErrorMessage','verifyRecoveryCode','verifyOtp({ email:value, token:code, type:"recovery" })','url.searchParams.set("auth", "recovery")','Password reset needs attention']) assert(cloud.includes(token),`V14.0.12 recovery redirect behavior safeguard missing: ${token}`);
...
assert(changelog.includes('## 14.0.12 · 2026-08-12'),"V14.0.12 changelog entry missing");
assert(version.name==='Dashboard Defaults & Rail Motion',"V14.0.12 version name mismatch");
assert(changelog.includes('## 13.0.15 · 2026-08-07')&&changelog.includes('Password-reset messaging avoids revealing'),"CHANGELOG V14.0.12 auth entry missing");
assert(version.cacheVersion.includes('v1412-dashboard-rail-r1'),"V14.0.12 auth cache revision mismatch");
assert(packageJson.scripts?.quality==='npm run inspect && npm run lint && npm run maintainability && npm run test',"V14.0.12 quality script mismatch");


// V14.0.12 iPhone input zoom prevention safeguards.
assert(version.name==="Dashboard Defaults & Rail Motion","V14.0.12 version name mismatch");
assert(version.cacheVersion.includes("v1412-dashboard-rail-r1"),"V14.0.12 cache revision mismatch");
assert(read("IPHONE_INPUT_ZOOM_VALIDATION_V13_0_17.md").includes("16px"),"V14.0.12 input-zoom guidance missing");
...
]) assert(html.includes(token),`V14.0.12 phone input safeguard missing: ${token}`);
assert(!/<meta[^>]+name=["']viewport["'][^>]+user-scalable\s*=\s*no/i.test(html),"V14.0.12 must not disable user pinch zoom");
assert(!/<meta[^>]+name=["']viewport["'][^>]+maximum-scale\s*=\s*1/i.test(html),"V14.0.12 must not lock maximum zoom scale");
assert(exists("IPHONE_INPUT_ZOOM_VALIDATION_V13_0_17.md"),"V14.0.12 input zoom validation report missing");

// V14.0.12 UI accessibility and signed-out Settings reliability safeguards.
for(const token of [
...
]) assert(html.includes(token),`V14.0.12 visual safeguard missing: ${token}`);
for(const token of [
...
]) assert(html.includes(token),`V14.0.12 interface safeguard missing: ${token}`);
for(const token of [
  "[data-help-key]",
...
]) assert(privacyLock.includes(token),`V14.0.12 privacy safeguard missing: ${token}`);
...
]) assert(browserCoverage.includes(token),`V14.0.12 browser coverage missing: ${token}`);
assert(readme.includes("**V14.0.12 · Dashboard Defaults & Rail Motion**"),"README V14.0.12 release notes missing");
for(const token of [
...
]) assert(html.includes(token),`V14.0.12 history or Settings search safeguard missing: ${token}`);
for(const token of ["baseRedoLastChange", 'key === "y"', 'key === "z" && event.shiftKey']) assert(productivity.includes(token),`V14.0.12 redo shortcut safeguard missing: ${token}`);
for(const token of ['#settingsSearchButton', '#settingsSearchInput', '[data-settings-search-result]']) assert(privacyLock.includes(token),`V14.0.12 signed-out Settings search safeguard missing: ${token}`);

// V14.0.12 approved Dashboard arrangement and temporary compact-rail expansion.
...
]) assert(html.includes(token),`V14.0.12 Dashboard or rail behavior missing: ${token}`);
...
assert(changelog.includes("approved Dashboard card sequence")&&changelog.includes("saved pin state"),"V14.0.12 changelog scope is incomplete");

if(failures.length){
  console.error("V14.0.12 Dashboard Defaults & Rail Motion validation failed:\n"+failures.map(item=>`- ${item}`).join("\n"));
  process.exit(1);
}
console.log("V14.0.12 Dashboard Defaults & Rail Motion validation passed.");
