...
test("Talaan V2.0.1 source keeps the approved desktop UX quick wins", async () => {
  ...
  expect(index).toContain("<title>Talaan · V2.0.1</title>");
  ...
  expect(index).toContain("./productivity-tools.js?v=2.0.1-talaan2");

  expect(version.version).toBe("2.0.1");
  expect(version.schemaVersion).toBe(12);
  expect(version.cloudSchemaVersion).toBe(3);
  expect(version.cacheVersion).toBe("finance-v2-20260822-talaan-r2");
  expect(sw).toContain('const APP_VERSION = "2.0.1";');
  expect(sw).toContain('finance-v2-20260822-talaan-r2');
});
