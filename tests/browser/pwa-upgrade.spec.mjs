...
test("Talaan V2.1.0 registers the cache-qualified worker and clears stale Finance caches", async ({ page }) => {
  ...
  await expect.poll(async () => page.evaluate(() => navigator.serviceWorker.controller?.scriptURL || ""), { timeout:15000 }).toContain("v=2.1.0");
  const workerUrl = await page.evaluate(() => navigator.serviceWorker.controller?.scriptURL || "");
  expect(workerUrl).toContain("cache=finance-v2-20260825-payees-rules-r7");
