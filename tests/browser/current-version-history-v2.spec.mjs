...
test("website Version history shows only the current Talaan V2.1.0 release", async ({ page }) => {
  ...
  await expect(entries.locator("summary strong")).toHaveText("V2.1.0 · Talaan");

  const visibleText = await card.innerText();
  expect(visibleText.match(/V\d+\.\d+\.\d+/g) || []).toEqual(["V2.1.0"]);
