...
test("production Talaan V2.3.0 UI alignment uses the delivered final stylesheet", async ({ browser }) => {
  ...
  const dashboardCss = styles.findIndex(href => href.includes("dashboard-interactions.css?v=2.3.0-talaan1"));
  const uiCss = styles.findIndex(href => href.includes("ui-icon-alignment.css?v=2.3.0-talaan1"));
  expect(dashboardCss).toBeGreaterThanOrEqual(0);
  expect(uiCss).toBeGreaterThan(dashboardCss);
  expect(styles.filter(href => href.includes("ui-icon-alignment.css?v=2.3.0-talaan1"))).toHaveLength(1);
...
test("Talaan V2.3.0 header moves Dashboard customization into More tools", async ({ page }) => {
  await page.goto("http://127.0.0.1:3000/index.html?page=dashboard", { waitUntil:"networkidle" });
