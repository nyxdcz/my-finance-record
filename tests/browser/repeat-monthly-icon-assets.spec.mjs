...
test("repeat monthly control uses replaceable PNG artwork and click bounce", async ({ page, request }) => {
  ...
  for (const name of ["off", "on"]) {
    const response = await request.get(`http://127.0.0.1:3000/icons/repeat-monthly-${name}.png?v=2.1.0-talaan1`);
  ...
  await page.addStyleTag({ url:"http://127.0.0.1:3000/production-ui-audit.css?v=2.1.0-talaan1" });
