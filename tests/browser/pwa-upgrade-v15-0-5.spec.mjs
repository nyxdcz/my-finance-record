...
test("V15.2.20 registers the cache-qualified worker and clears stale Finance caches", async ({ page }) => {
  ...
  await expect.poll(async () => page.evaluate(() => navigator.serviceWorker.controller?.scriptURL || ""), { timeout:15000 }).toContain("v=15.2.20");
  const workerUrl = await page.evaluate(() => navigator.serviceWorker.controller?.scriptURL || "");
  expect(workerUrl).toContain("cache=finance-v15-20260821-compact-expense-stability-r56");
