...
const requiredFiles = [
  ...
  "Install_V15_0_2.command", "run_audit.sh", "eslint.config.js", "playwright.config.mjs",
  "tests/validate-v15-0-2.mjs", "tests/validate-expense-screenshot.mjs", "tests/expense-screenshot.spec.mjs", "tests/privacy-and-inputs.spec.mjs", "tests/check-maintainability.mjs"
];
...
if (pkg.version !== "15.0.2") fail(`Expected current package version 15.0.2, found ${pkg.version || "(missing)"}`);
if (!read("README.md").startsWith("# My Finance Records · V15.0.2")) fail("README release heading is not V15.0.2");
if (!read("CHANGELOG.md").startsWith("## 15.0.2 · 2026-08-15")) fail("CHANGELOG latest entry is not V15.0.2");
...
if (process.platform !== "win32") {
  for (const file of ["Install_V15_0_2.command", "run_audit.sh"]) {
    const patchWorkflowCreatingInstaller = file === "Install_V15_0_2.command" && process.env.GITHUB_WORKFLOW === "Agent V15.0.2 Release Patch";
    if (!patchWorkflowCreatingInstaller && (fs.statSync(path.join(root, file)).mode & 0o100) === 0) fail(`Executable entry point lost its user-executable bit: ${file}`);
  }
...
console.log("Repository inspection passed: V15.0.2 release files, local paths, deploy paths, package metadata, permissions, and public sync configuration are consistent.");
