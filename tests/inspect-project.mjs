...
const requiredFiles = [
  ...
  "Install_V14_0_8.command", "run_audit.sh", "eslint.config.js", "playwright.config.mjs",
  "tests/validate-v14-0-8.mjs", "tests/privacy-and-inputs.spec.mjs", "tests/check-maintainability.mjs"
];
...
if (pkg.version !== "14.0.8") fail(`Expected current package version 14.0.8, found ${pkg.version || "(missing)"}`);
if (!read("README.md").startsWith("# My Finance Records · V14.0.8 PWA")) fail("README release heading is not V14.0.8");
...
if (process.platform !== "win32") {
  for (const file of ["Install_V14_0_8.command", "run_audit.sh"]) {
