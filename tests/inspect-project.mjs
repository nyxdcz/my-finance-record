...
const requiredFiles = [
  ...
  "Install_V14_0_9.command", "run_audit.sh", "eslint.config.js", "playwright.config.mjs",
  "tests/validate-v14-0-9.mjs", "tests/privacy-and-inputs.spec.mjs", "tests/check-maintainability.mjs"
];
...
if (pkg.version !== "14.0.9") fail(`Expected current package version 14.0.9, found ${pkg.version || "(missing)"}`);
if (!read("README.md").startsWith("# My Finance Records · V14.0.9")) fail("README release heading is not V14.0.9");
...
if (process.platform !== "win32") {
  for (const file of ["Install_V14_0_9.command", "run_audit.sh"]) {
