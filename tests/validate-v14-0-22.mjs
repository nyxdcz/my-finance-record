...
const installer=read("Install_V14_0_22.command");
...
assert(html.includes('<title>My Finance Records · V14.0.22</title>'),"HTML title mismatch");
assert(html.includes('const APP_VERSION = "14.0.22";'),"HTML APP_VERSION mismatch");
assert(html.includes('const APP_RELEASE_NAME = "Marquee & Sidebar Stability";'),"HTML release name mismatch");
...
assert(html.includes('{"version":"V14.0.22","title":"Marquee & Sidebar Stability"'),"in-app V14.0.22 history entry missing");
...
assert(html.includes('./projects-calendar-v13.0.20.js?v=14.0.22')&&html.includes('./projects-calendar-v13.0.20.css?v=14.0.22'),"Project Agenda assets are not version-pinned in HTML");
assert(worker.includes('asset("./projects-calendar-v13.0.20.js?v=14.0.22")')&&worker.includes('asset("./projects-calendar-v13.0.20.css?v=14.0.22")'),"Project Agenda assets are not version-pinned in the offline shell");
assert(html.includes('./cloud-conflict-review.js?v=14.0.22')&&worker.includes('asset("./cloud-conflict-review.js?v=14.0.22")'),"Cloud conflict review module is not loaded and precached");
assert(html.includes('./cloud-conflict-resolution.js?v=14.0.22')&&worker.includes('asset("./cloud-conflict-resolution.js?v=14.0.22")'),"Cloud conflict resolution module is not loaded and precached");
...
assert(read("index.html").includes('./app.css?v=14.0.22')&&worker.includes('asset("./app.css?v=14.0.22")'),"extracted application CSS is not versioned and precached");
...
assert(worker.includes('const APP_VERSION = "14.0.22";'),"service-worker version mismatch");
assert(worker.includes(`const CACHE_VERSION = "${version.cacheVersion}";`),"service-worker cache mismatch");
assert(worker.includes('asset("./security-profiles.js?v=14.0.22")')&&worker.includes('asset("./security-profiles.css?v=14.0.22")'),"profile assets missing from PWA shell");
...
assert(packageJson.version==="14.0.22"&&packageLock.version==="14.0.22","package version mismatch");
...
assert(worker.includes('asset("./privacy-lock.js?v=14.0.22")'),"privacy module missing from service-worker shell");
assert(workflow.includes("privacy-lock.js"),"GitHub Pages workflow does not deploy privacy module");
assert(exists("SIGNED_OUT_PRIVACY_LOCK_VALIDATION_V13_0_18.md"),"V14.0.22 privacy validation report missing");
...
const recentReadmeVersions=["V14.0.22","V14.0.21","V14.0.20","V14.0.19","V14.0.18"];
...
assert(exists("Install_V14_0_22.command")&&(fs.statSync(path.join(root,"Install_V14_0_22.command")).mode&0o100)!==0,"V14.0.22 installer is missing or not executable");
assert((fs.statSync(path.join(root,"run_audit.sh")).mode&0o100)!==0,"run_audit.sh is not executable");
assert(installer.includes("npm ci --ignore-scripts")&&installer.includes("npm run quality")&&installer.includes("git diff --check"),"V14.0.22 installer validation flow is incomplete");
...
assert(readme.startsWith("# My Finance Records · V14.0.22"),"README release heading mismatch");
...
const cloudSandbox={
  ...
  data:structuredClone(sampleData),APP_VERSION:"14.0.22",normalizeData:value=>value,saveData(){return true;},renderAll(){},showToast(message,tone){cloudSandbox.__lastToast={message,tone};},STORAGE_KEY:"finance",setInterval(){return 0;},clearInterval(){},setTimeout(){return 0;},clearTimeout(){}
};
...
]) assert(cloud.includes(token),`V14.0.22 cross-device sync safeguard missing: ${token}`);
for(const token of ["function reconcileUnqueuedLocalChanges", "!profileCanWrite()", "Recovered unqueued local Finance changes", "const recovered=reconcileUnqueuedLocalChanges()", "previously missed Finance record"]) assert(cloud.includes(token),`V14.0.22 existing-state repair safeguard missing: ${token}`);
...
for(const token of ['const FOREGROUND_POLL_MS = 5 * 60 * 1000','pull("foreground-poll")','pull("realtime-recovery")','function scheduleRealtimeRecovery','function noteRealtimeSubscribed']) assert(cloudLifecycle.includes(token),`V14.0.22 lifecycle controller safeguard missing: ${token}`);
assert(html.includes('./cloud-sync-lifecycle.js?v=14.0.22')&&worker.includes('asset("./cloud-sync-lifecycle.js?v=14.0.22")')&&workflow.includes("cloud-sync-lifecycle.js"),"V14.0.22 lifecycle module is not loaded, cached, and deployed");
assert(changelog.includes("every shared Finance persistence event")&&changelog.includes("30-second visible-device polling fallback"),"V14.0.22 sync changelog scope is incomplete");
assert(changelog.includes("pre-pull reconciliation pass")&&changelog.includes("Metrobank-to-GoTyme"),"V14.0.22 existing-state repair changelog scope is incomplete");
...
assert(html.includes('id="settingsOverviewAppStatus">Version 14.0.22<'),"Settings overview version was not updated");
...
]) assert(html.includes(token),`V14.0.22 auth UI safeguard missing: ${token}`);
for(const token of [
...
]) assert(cloud.includes(token),`V14.0.22 auth behavior safeguard missing: ${token}`);
assert(changelog.includes('Forgot password')&&changelog.includes('Test cloud connection'),"CHANGELOG auth recovery notes missing");
for(const token of ['id="cloudRecoveryHelpCard"','id="cloudRecoveryEmail"','id="cloudRecoveryCode"','id="cloudRecoveryResend"','id="cloudVerifyRecoveryCode"','id="cloudRecoveryBackToSignIn"']) assert(html.includes(token),`V14.0.22 recovery redirect UI safeguard missing: ${token}`);
for(const token of ['parsePasswordRecoveryUrl','recoveryErrorMessage','verifyRecoveryCode','verifyOtp({ email:value, token:code, type:"recovery" })','url.searchParams.set("auth", "recovery")','Password reset needs attention']) assert(cloud.includes(token),`V14.0.22 recovery redirect behavior safeguard missing: ${token}`);
...
]) assert(html.includes(token),`V14.0.22 phone input safeguard missing: ${token}`);
assert(!/<meta[^>]+name=["']viewport["'][^>]+user-scalable\s*=\s*no/i.test(html),"V14.0.22 must not disable user pinch zoom");
assert(!/<meta[^>]+name=["']viewport["'][^>]+maximum-scale\s*=\s*1/i.test(html),"V14.0.22 must not lock maximum zoom scale");
assert(exists("IPHONE_INPUT_ZOOM_VALIDATION_V13_0_17.md"),"V14.0.22 input zoom validation report missing");

// V14.0.22 UI accessibility and signed-out Settings reliability safeguards.
for(const token of [
...
]) assert(html.includes(token),`V14.0.22 visual safeguard missing: ${token}`);
for(const token of [
...
]) assert(html.includes(token),`V14.0.22 interface safeguard missing: ${token}`);
for(const token of [
  "[data-help-key]",
...
]) assert(privacyLock.includes(token),`V14.0.22 privacy safeguard missing: ${token}`);
...
]) assert(browserCoverage.includes(token),`V14.0.22 browser coverage missing: ${token}`);
assert(readme.includes("**V14.0.22 · Marquee & Sidebar Stability**"),"README V14.0.22 release notes missing");
for(const token of [
...
]) assert(html.includes(token),`V14.0.22 history or Settings search safeguard missing: ${token}`);
for(const token of ["baseRedoLastChange", 'key === "y"', 'key === "z" && event.shiftKey']) assert(productivity.includes(token),`V14.0.22 redo shortcut safeguard missing: ${token}`);
for(const token of ['#settingsSearchButton', '#settingsSearchInput', '[data-settings-search-result]']) assert(privacyLock.includes(token),`V14.0.22 signed-out Settings search safeguard missing: ${token}`);

// V14.0.22 approved Dashboard arrangement and temporary compact-rail expansion.
...
]) assert(html.includes(token),`V14.0.22 Dashboard or rail behavior missing: ${token}`);
...
assert(changelog.includes("approved Dashboard card sequence")&&changelog.includes("saved pin state"),"V14.0.22 changelog scope is incomplete");

// V14.0.22 primary navigation labels and Philippine-time appearance schedule.
for(const [page,label] of Object.entries({dashboard:"Overview",money:"Finance",projects:"Work",reports:"Insights"})) {
  assert(html.includes(`data-page="${page}" data-nav-label="${label}"`),`V14.0.22 sidebar label missing for ${page}`);
}
for(const token of [
...
]) assert(html.includes(token),`V14.0.22 Philippine-time theme safeguard missing: ${token}`);
...
assert(changelog.includes("Asia/Manila")&&changelog.includes("7:01 AM")&&changelog.includes("6:00 PM"),"V14.0.22 Manila schedule documentation is incomplete");

// V14.0.22 duplicate sidebar group-heading removal.
...
]) assert(browserCoverage.includes(token),`V14.0.22 responsive sidebar coverage missing: ${token}`);
assert(changelog.includes("duplicated uppercase Overview, Finance, Work, and Insights headings"),"V14.0.22 sidebar cleanup documentation is incomplete");

// V14.0.22 accessible Dashboard interaction system.
...
assert(changelog.includes("one-week Dashboard calendar marquee")&&changelog.includes("ARIA live announcements")&&changelog.includes('aria-haspopup="menu"'),"V14.0.22 changelog scope is incomplete");

// V14.0.22 Finance marquee, native focus ring, and compact component roles.
...
for(const token of ['#financeWeekMarqueeTrack > .dashboard-week-marquee-group','Remove Category: Subscriptions filter','getComputedStyle(node).outlineStyle','.record-row .ui-tag','.record-row .ui-pill']) assert(browserCoverage.includes(token),`V14.0.22 browser coverage missing: ${token}`);
assert(changelog.includes("native browser focus rings")&&changelog.includes("active-filter Chips")&&changelog.includes("category Tags"),"V14.0.22 changelog scope is incomplete");

// V14.0.22 43px marquee row, phone removal, and stable pointer-exit sidebar.
for(const token of ['id="dashboardWeekMarquee"','id="dashboard-title"','id="incomeFinanceWeekMarquee"','id="paidFinanceWeekMarquee"']) assert(html.includes(token),`V14.0.22 marquee placement missing: ${token}`);
...
for(const token of ['height:43px; min-height:43px; max-height:43px','finance-workspace-marquee-row','> .money-workspace-switcher { position:static; top:auto; flex:0 0 auto; height:43px; min-height:43px; max-height:43px; box-sizing:border-box','> .finance-week-marquee { flex:1 1 auto','.dashboard-week-marquee { display:none!important; }']) assert(css.includes(token),`V14.0.22 marquee layout safeguard missing: ${token}`);
for(const [page,file] of Object.entries({dashboard:"sidebar-overview.png",money:"sidebar-finance.png",projects:"sidebar-work.png",settings:"sidebar-settings.png"})) assert(html.includes(`data-page="${page}" data-nav-label`)&&html.includes(`src="./icons/${file}"`),`V14.0.22 supplied ${page} icon missing`);
...
console.log("V14.0.22 Marquee & Sidebar Stability validation passed.");
