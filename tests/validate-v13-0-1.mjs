...
const uiValidation=read("MACBOOK_IPHONE_UI_VALIDATION_V13_0_1.md");
const version=JSON.parse(read("version.json"));
const packageJson=JSON.parse(read("package.json"));
const packageLock=JSON.parse(read("package-lock.json"));

assert(version.version==="13.0.1","version.json is not V13.0.1");
...
assert(html.includes('<title>My Finance Records · V13.0.1</title>'),"HTML title mismatch");
assert(html.includes('const APP_VERSION = "13.0.1";'),"HTML APP_VERSION mismatch");
...
assert(html.includes('{"version": "V13.0.1", "title": "MacBook & iPhone UI Stabilization"'),"in-app V13.0.1 history entry missing");
...
assert(worker.includes('const APP_VERSION = "13.0.1";'),"service-worker version mismatch");
...
assert(packageJson.version==="13.0.1"&&packageLock.version==="13.0.1","package version mismatch");
assert(packageJson.scripts?.quality==="node tests/validate-v13-0-1.mjs","quality script mismatch");
assert(readme.startsWith("# My Finance Records · V13.0.1 PWA"),"README heading mismatch");
...
assert(changelog.includes("## 13.0.1 · 2026-08-07"),"CHANGELOG V13 entry missing");
...
assert(uiValidation.includes("393 × 852")&&uiValidation.includes("44 × 44px"),"V13.0.1 UI validation scope incomplete");
...
const syntaxFiles=[
  "security-profiles.js","cloud-sync.js","account-ledger.js","budget-planning.js","reports-insights.js","productivity-tools.js","reminders-alerts.js","sw.js","tests/validate-v13-0-1.mjs"
];
...
inlineScripts.forEach((code,index)=>{
  const temp=path.join(root,`.v13001-inline-${index}.js`);
...
for(const token of [
  "mobile-topbar-tools","mobileQuickActionButton","mobileUtilityMenu","workspace-label-mobile","mobile-filter-count",
  "--mobile-topbar-offset","grid-template-areas:\"title tools\" \"month month\"","centerScrollableTab",
  "syncMobileFilterPanels","updateMobileFilterBadges","reportSectionNav"
]) assert(html.includes(token),`V13.0.1 responsive UI safeguard missing: ${token}`);
assert(!html.includes('id="mobileAddExpenseButton"'),"obsolete floating mobile Add Expense control remains");
...
if(failures.length){
  console.error("V13.0.1 MacBook & iPhone UI Stabilization validation failed:\n"+failures.map(item=>`- ${item}`).join("\n"));
  process.exit(1);
}
console.log("V13.0.1 MacBook & iPhone UI Stabilization validation passed.");
