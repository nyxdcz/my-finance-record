...
assert(version.version==="13.0.7","version.json is not V13.0.7");
...
assert(html.includes('<title>My Finance Records · V13.0.7</title>'),"HTML title mismatch");
assert(html.includes('const APP_VERSION = "13.0.7";'),"HTML APP_VERSION mismatch");
...
assert(html.includes('{"version": "V13.0.7", "title": "Budget Plan Compact Layout"'),"in-app V13.0.7 history entry missing");
...
assert(worker.includes('const APP_VERSION = "13.0.7";'),"service-worker version mismatch");
...
assert(packageJson.version==="13.0.7"&&packageLock.version==="13.0.7","package version mismatch");
assert(packageJson.scripts?.quality==="node tests/validate-v13-0-7.mjs","quality script mismatch");
...
assert(exists("PROJECT_REVISION_CYCLES_VALIDATION_V13_0_6.md"),"V13.0.7 project revision validation report missing");
assert(exists("FILE_INSPECTION_AND_FIXES_VALIDATION_V13_0_5.md"),"V13.0.5 installer validation report missing");
assert(readme.includes("My_Finance_Records_V13_0_7_File_Inspection_and_Fixes_Installer.zip")&&readme.includes("Install_V13_0_7.command"),"README V13.0.7 installer filenames are stale");
...
assert(readme.startsWith("# My Finance Records · V13.0.7 PWA"),"README heading mismatch");
...
assert(changelog.includes("## 13.0.7 · 2026-08-07"),"CHANGELOG V13.0.7 entry missing");
...
const syntaxFiles=[
  "security-profiles.js","cloud-sync.js","account-ledger.js","budget-planning.js","reports-insights.js","productivity-tools.js","reminders-alerts.js","sw.js","tests/validate-v13-0-7.mjs"
];
...
assert(version.cacheVersion.includes("v13007-budget-plan-compact-r1"),"V13.0.7 cache revision mismatch");
...
// V13.0.5 Dashboard calendar deduplication safeguards retained in V13.0.7.
for(const token of [
...
]) assert(html.includes(token),`V13.0.5 calendar safeguard missing after V13.0.7: ${token}`);
...
// V13.0.7 Project revision-cycle safeguards.
for(const token of [
...
]) assert(html.includes(token),`V13.0.7 project revision safeguard missing: ${token}`);
assert(html.includes('<option>In revision</option>'),"In revision project status option missing");
assert(changelog.includes("numbered project revision cycles")&&readme.includes("Revision 1, Revision 2"),"V13.0.7 project revision documentation incomplete");
assert(exists("PROJECT_REVISION_CYCLES_VALIDATION_V13_0_6.md"),"V13.0.7 project revision report missing");
assert(exists("FILE_INSPECTION_AND_FIXES_VALIDATION_V13_0_6.md"),"V13.0.6 installer validation report missing");

assert(html.includes('id="settingsOverviewAppStatus">Version 13.0.7<'),"Settings overview version was not updated");
...
// V13.0.7 Monthly budget-plan compact-layout safeguards.
...
]) assert(budgetSource.includes(token),`V13.0.7 budget-plan collapse safeguard missing: ${token}`);
for (const token of [
...
]) assert(budgetCss.includes(token),`V13.0.7 compact budget CSS safeguard missing: ${token}`);
assert(readme.includes("V13.0.7 · Budget Plan Compact Layout")&&changelog.includes("complete Monthly budget plan"),"V13.0.7 documentation incomplete");
assert(exists("BUDGET_PLAN_COMPACT_LAYOUT_VALIDATION_V13_0_7.md"),"V13.0.7 budget layout validation report missing");
assert(exists("FILE_INSPECTION_AND_FIXES_VALIDATION_V13_0_7.md"),"V13.0.7 installer validation report missing");
assert(version.name==="Budget Plan Compact Layout","V13.0.7 version name mismatch");

if(failures.length){
  console.error("V13.0.7 Budget Plan Compact Layout validation failed:\n"+failures.map(item=>`- ${item}`).join("\n"));
  process.exit(1);
}
console.log("V13.0.7 Budget Plan Compact Layout validation passed.");
