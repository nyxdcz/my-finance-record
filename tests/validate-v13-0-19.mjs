...
assert(version.version==="13.0.19","version.json is not V13.0.19");
...
assert(html.includes('<title>My Finance Records · V13.0.19</title>'),"HTML title mismatch");
assert(html.includes('const APP_VERSION = "13.0.19";'),"HTML APP_VERSION mismatch");
...
assert(worker.includes('const APP_VERSION = "13.0.19";'),"service-worker version mismatch");
assert(worker.includes(`const CACHE_VERSION = "${version.cacheVersion}";`),"service-worker cache mismatch");
assert(worker.includes('asset("./security-profiles.js?v=13.0.19")')&&worker.includes('asset("./security-profiles.css?v=13.0.19")'),"profile assets missing from PWA shell");
...
assert(packageJson.version==="13.0.19"&&packageLock.version==="13.0.19","package version mismatch");
...
assert(worker.includes('asset("./privacy-lock.js?v=13.0.19")'),"privacy module missing from service-worker shell");
assert(workflow.includes("privacy-lock.js"),"GitHub Pages workflow does not deploy privacy module");
assert(exists("SIGNED_OUT_PRIVACY_LOCK_VALIDATION_V13_0_18.md"),"signed-out privacy validation report missing");

assert(packageJson.scripts?.quality==="node tests/validate-v13-0-19.mjs","quality script mismatch");
...
assert(readme.startsWith("# My Finance Records · V13.0.19 PWA"),"README V13.0.15 heading mismatch");
...
if(profileApi&&profileInternals){
  ...
  assert(profileInternals.normalizeAutoLockMinutes("forever")===null,"Forever auto-lock was not stored as a non-timed setting");
  assert(profileInternals.autoLockIsForever({autoLockMinutes:null})===true,"Forever auto-lock was not recognized");
  assert(profileInternals.normalizeAutoLockMinutes("60")===60&&profileInternals.normalizeAutoLockMinutes("invalid")===15,"Timed auto-lock normalization regressed");
...
assert(version.cacheVersion.includes("v13019-forever-auto-lock-r1"),"V13.0.19 revision deadline cache revision mismatch");
...
for(const token of [
  ...
  'New deadline for this revision','projectRevisionDeadlineHelp','function syncProjectRevisionDeadlineFields','deadline.min = requestedDate',
...
assert(html.includes('id="settingsOverviewAppStatus">Version 13.0.19<'),"Settings overview version was not updated");
...
assert(version.name==="Forever Device Auto-Lock", "V13.0.15 version name mismatch");
...
assert(version.cacheVersion.includes("v13019-forever-auto-lock-r1"),"V13.0.19 revision deadline cache revision mismatch");
assert(readme.startsWith("# My Finance Records · V13.0.19 PWA")&&readme.includes("Brave PWA Install Flow"),"V13.0.15 README metadata missing");
assert(changelog.includes("## 13.0.15 · 2026-08-07"),"V13.0.15 changelog missing");
assert(html.includes('?v=13.0.19')&&worker.includes('?v=13.0.19'),"Version-pinned V13.0.15 assets missing");
...
]) assert(html.includes(token),`V13.0.19 auth UI safeguard missing: ${token}`);
for(const token of [
...
]) assert(cloud.includes(token),`V13.0.19 auth behavior safeguard missing: ${token}`);
assert(readme.includes('Forgot password?')&&readme.includes('Test cloud connection'),"README V13.0.19 auth recovery notes missing");
for(const token of ['id="cloudRecoveryHelpCard"','id="cloudRecoveryEmail"','id="cloudRecoveryCode"','id="cloudRecoveryResend"','id="cloudVerifyRecoveryCode"','id="cloudRecoveryBackToSignIn"']) assert(html.includes(token),`V13.0.19 recovery redirect UI safeguard missing: ${token}`);
for(const token of ['parsePasswordRecoveryUrl','recoveryErrorMessage','verifyRecoveryCode','verifyOtp({ email:value, token:code, type:"recovery" })','url.searchParams.set("auth", "recovery")','Password reset needs attention']) assert(cloud.includes(token),`V13.0.19 recovery redirect behavior safeguard missing: ${token}`);
assert(readme.includes('?auth=recovery')&&readme.includes('{{ .Token }}'),"README V13.0.19 recovery redirect/code instructions missing");
assert(changelog.includes('## 13.0.19 · 2026-08-08'),"V13.0.19 changelog entry missing");
assert(version.name==='Forever Device Auto-Lock',"V13.0.19 version name mismatch");
assert(changelog.includes('## 13.0.15 · 2026-08-07')&&changelog.includes('Password-reset messaging avoids revealing'),"CHANGELOG V13.0.19 auth entry missing");
assert(version.cacheVersion.includes('v13019-forever-auto-lock-r1'),"V13.0.19 revision deadline cache revision mismatch");
assert(packageJson.scripts?.quality==='node tests/validate-v13-0-19.mjs',"V13.0.19 quality script mismatch");

// V13.0.19 Forever device auto-lock safeguards.
assert(version.name==="Forever Device Auto-Lock","V13.0.19 version name mismatch");
assert(html.includes('"version":"V13.0.19","title":"Forever Device Auto-Lock"'),"V13.0.19 in-app version history entry missing");
assert(profiles.includes('value="forever"')&&profiles.includes('function normalizeAutoLockMinutes')&&profiles.includes('function autoLockIsForever'),"Forever auto-lock option or behavior is missing");
assert(profiles.includes('if (autoLockIsForever(config)) return;'),"Forever auto-lock still schedules an inactivity timer");
assert(changelog.includes('## 13.0.19 · 2026-08-08')&&readme.includes('## V13.0.19 · Forever Device Auto-Lock'),"V13.0.19 release notes are missing");


// V13.0.19 iPhone input zoom prevention safeguards.
assert(version.name==="Forever Device Auto-Lock","V13.0.19 version name mismatch");
assert(version.cacheVersion.includes("v13019-forever-auto-lock-r1"),"V13.0.19 revision deadline cache revision mismatch");
assert(readme.includes("iPhone Input Zoom Prevention")&&readme.includes("minimum **16px**"),"README V13.0.19 input-zoom guidance missing");
...
]) assert(html.includes(token),`V13.0.19 phone input safeguard missing: ${token}`);
assert(!/<meta[^>]+name=["']viewport["'][^>]+user-scalable\s*=\s*no/i.test(html),"V13.0.19 must not disable user pinch zoom");
assert(!/<meta[^>]+name=["']viewport["'][^>]+maximum-scale\s*=\s*1/i.test(html),"V13.0.19 must not lock maximum zoom scale");
assert(exists("DEVICE_LOCK_FOREVER_VALIDATION_V13_0_19.md"),"V13.0.19 input zoom validation report missing");

if(failures.length){
  console.error("V13.0.19 Forever Device Auto-Lock validation failed:\n"+failures.map(item=>`- ${item}`).join("\n"));
  process.exit(1);
}
console.log("V13.0.19 Forever Device Auto-Lock validation passed.");
