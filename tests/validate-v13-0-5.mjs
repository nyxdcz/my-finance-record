...
assert(version.version==="13.0.5","version.json is not V13.0.5");
...
assert(html.includes('<title>My Finance Records · V13.0.5</title>'),"HTML title mismatch");
assert(html.includes('const APP_VERSION = "13.0.5";'),"HTML APP_VERSION mismatch");
...
assert(html.includes('{"version": "V13.0.5", "title": "Dashboard Calendar Deduplication"'),"in-app V13.0.5 history entry missing");
...
assert(worker.includes('const APP_VERSION = "13.0.5";'),"service-worker version mismatch");
...
assert(packageJson.version==="13.0.5"&&packageLock.version==="13.0.5","package version mismatch");
assert(packageJson.scripts?.quality==="node tests/validate-v13-0-5.mjs","quality script mismatch");
...
assert(readme.includes("My_Finance_Records_V13_0_5_File_Inspection_and_Fixes_Installer.zip")&&readme.includes("Install_V13_0_5.command"),"README V13.0.5 installer filenames are stale");
...
assert(readme.startsWith("# My Finance Records · V13.0.5 PWA"),"README heading mismatch");
...
assert(changelog.includes("## 13.0.5 · 2026-08-07"),"CHANGELOG V13.0.5 entry missing");
...
const syntaxFiles=[
  "security-profiles.js","cloud-sync.js","account-ledger.js","budget-planning.js","reports-insights.js","productivity-tools.js","reminders-alerts.js","sw.js","tests/validate-v13-0-5.mjs"
];
...
assert(version.cacheVersion.includes("v13005-calendar-dedup-r1"),"V13.0.5 cache revision mismatch");
...
// V13.0.5 Dashboard calendar deduplication safeguards.
for(const token of [
  ...
  "sourceKey:`project|${project.id}|${date}|deadline`",
  "sourceKey:`payment|${paymentSource}|${date}|received`"
]) assert(html.includes(token),`V13.0.5 calendar safeguard missing: ${token}`);
...
assert(html.includes('id="settingsOverviewAppStatus">Version 13.0.5<'),"Settings overview version was not updated");
...
if(failures.length){
  console.error("V13.0.5 Dashboard Calendar Deduplication validation failed:\n"+failures.map(item=>`- ${item}`).join("\n"));
  process.exit(1);
}
console.log("V13.0.5 Dashboard Calendar Deduplication validation passed.");
