...
assert(version.version==="13.0.13","version.json is not V13.0.13");
...
assert(html.includes('<title>My Finance Records · V13.0.13</title>'),"HTML title mismatch");
assert(html.includes('const APP_VERSION = "13.0.13";'),"HTML APP_VERSION mismatch");
...
assert(worker.includes('const APP_VERSION = "13.0.13";'),"service-worker version mismatch");
assert(worker.includes(`const CACHE_VERSION = "${version.cacheVersion}";`),"service-worker cache mismatch");
assert(worker.includes('asset("./security-profiles.js?v=13.0.13")')&&worker.includes('asset("./security-profiles.css?v=13.0.13")'),"profile assets missing from PWA shell");
...
assert(packageJson.version==="13.0.13"&&packageLock.version==="13.0.13","package version mismatch");
assert(packageJson.scripts?.quality==="node tests/validate-v13-0-13.mjs","quality script mismatch");
...
assert(readme.includes("My_Finance_Records_V13_0_13_File_Inspection_and_Fixes_Installer.zip")&&readme.includes("Install_V13_0_13.command"),"README V13.0.13 installer filenames are stale");
...
assert(readme.startsWith("# My Finance Records · V13.0.13 PWA"),"README V13.0.13 heading mismatch");
...
assert(changelog.includes("## 13.0.12 · 2026-08-07")&&changelog.includes("## 13.0.10 · 2026-08-07"),"CHANGELOG V13.0.13 entry or V13.0.10 history missing");
...
assert(version.cacheVersion.includes("v13013-spend-transaction-r1"),"V13.0.13 cache revision mismatch");
...
assert(html.includes('id="settingsOverviewAppStatus">Version 13.0.13<'),"Settings overview version was not updated");
...
assert(version.name==="Record Spending Transaction Hotfix", "V13.0.13 version name mismatch");
...
// V13.0.13 interaction reliability and project mobile cleanup safeguards.
for(const token of [
...
]) assert(html.includes(token)||ledgerSource.includes(token),`V13.0.13 interaction/UI safeguard missing: ${token}`);
...
assert(readme.includes("V13.0.12 · Spend Reliability & Phone Budget Compaction")&&changelog.includes("## 13.0.12 · 2026-08-07"),"V13.0.13 documentation missing");
assert(exists("UI_INTERACTION_RELIABILITY_VALIDATION_V13_0_11.md"),"V13.0.11 validation report missing");
assert(exists("RECORD_SPENDING_TRANSACTION_VALIDATION_V13_0_13.md"),"V13.0.13 transaction validation report missing");


// V13.0.13 Record Spending transaction hotfix safeguards.
...
]) assert(ledgerV1313.includes(token),`V13.0.13 transaction safeguard missing: ${token}`);
...
assert(version.cacheVersion.includes("v13013-spend-transaction-r1"),"V13.0.13 cache revision mismatch");
assert(readme.startsWith("# My Finance Records · V13.0.13 PWA")&&readme.includes("Record Spending Transaction Hotfix"),"V13.0.13 README metadata missing");
assert(changelog.includes("## 13.0.13 · 2026-08-07"),"V13.0.13 changelog missing");
assert(html.includes('?v=13.0.13')&&worker.includes('?v=13.0.13'),"Version-pinned V13.0.13 assets missing");
assert(ledgerV1313.includes('const stored = JSON.parse(storedRaw);'),"V13.0.13 storage verification must inspect the raw persisted ledger");
...
if(failures.length){
  console.error("V13.0.13 Record Spending Transaction Hotfix validation failed:\n"+failures.map(item=>`- ${item}`).join("\n"));
  process.exit(1);
}
console.log("V13.0.13 Record Spending Transaction Hotfix validation passed.");
