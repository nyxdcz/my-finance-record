...
assert.match(runtimeCompat, /expense-screenshot-parser\.js\?v=2\.1\.0-talaan1/, "screenshot parser should use the current Talaan runtime query");
assert.match(runtimeCompat, /expense-screenshot-detect\.js\?v=2\.1\.0-talaan1/, "local screenshot detector should use the current Talaan runtime query");
assert.match(runtimeCompat, /expense-screenshot-ai\.js\?v=2\.1\.0-talaan1/, "optional AI screenshot detector should use the current Talaan runtime query");
...
assert.equal(version.cacheVersion, "finance-v2-20260825-payees-rules-r7");
assert.ok(worker.includes(version.cacheVersion), "service worker cache generation must match Talaan V2.1.0 while preserving screenshot detection");
assert.match(worker, /expense-screenshot-parser\.js\?v=2\.1\.0-talaan1/, "service worker should precache the screenshot parser on the current query");
assert.match(worker, /expense-screenshot-detect\.js\?v=2\.1\.0-talaan1/, "service worker should precache the local screenshot detector on the current query");
assert.match(worker, /expense-screenshot-ai\.js\?v=2\.1\.0-talaan1/, "service worker should precache the optional AI client on the current query");

console.log("Expense screenshot local and optional AI detector validation passed under Talaan V2.1.0.");
