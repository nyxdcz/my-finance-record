...
assert(version.version==="13.0.12","version.json is not V13.0.12");
...
assert(html.includes('<title>My Finance Records · V13.0.12</title>'),"HTML title mismatch");
assert(html.includes('const APP_VERSION = "13.0.12";'),"HTML APP_VERSION mismatch");
...
assert(worker.includes('const APP_VERSION = "13.0.12";'),"service-worker version mismatch");
assert(worker.includes(`const CACHE_VERSION = "${version.cacheVersion}";`),"service-worker cache mismatch");
assert(worker.includes('asset("./security-profiles.js?v=13.0.12")')&&worker.includes('asset("./security-profiles.css?v=13.0.12")'),"profile assets missing from PWA shell");
...
assert(packageJson.version==="13.0.12"&&packageLock.version==="13.0.12","package version mismatch");
assert(packageJson.scripts?.quality==="node tests/validate-v13-0-12.mjs","quality script mismatch");
...
assert(readme.includes("My_Finance_Records_V13_0_12_File_Inspection_and_Fixes_Installer.zip")&&readme.includes("Install_V13_0_12.command"),"README V13.0.12 installer filenames are stale");
...
assert(readme.startsWith("# My Finance Records · V13.0.12 PWA"),"README V13.0.12 heading mismatch");
...
assert(changelog.includes("## 13.0.12 · 2026-08-07")&&changelog.includes("## 13.0.10 · 2026-08-07"),"CHANGELOG V13.0.12 entry or V13.0.10 history missing");
...
assert(version.cacheVersion.includes("v13012-spend-budget-r1"),"V13.0.10 cache revision mismatch");
...
assert(html.includes('id="settingsOverviewAppStatus">Version 13.0.12<'),"Settings overview version was not updated");
...
assert(version.name==="Spend Reliability & Phone Budget Compaction", "V13.0.12 version name mismatch");
...
// V13.0.12 interaction reliability and project mobile cleanup safeguards.
for(const token of [
...
]) assert(html.includes(token)||ledgerSource.includes(token),`V13.0.12 interaction/UI safeguard missing: ${token}`);
...
assert(readme.includes("V13.0.12 · Spend Reliability & Phone Budget Compaction")&&changelog.includes("## 13.0.12 · 2026-08-07"),"V13.0.12 documentation missing");
assert(exists("UI_INTERACTION_RELIABILITY_VALIDATION_V13_0_11.md"),"V13.0.12 validation report missing");

if(failures.length){
  console.error("V13.0.12 Spend Reliability & Phone Budget Compaction validation failed:\n"+failures.map(item=>`- ${item}`).join("\n"));
  process.exit(1);
}
console.log("V13.0.12 Spend Reliability & Phone Budget Compaction validation passed.");
...
const htmlV1312=fs.readFileSync("index.html","utf8"); const ledgerV1312=fs.readFileSync("account-ledger.js","utf8"); const swV1312=fs.readFileSync("sw.js","utf8");
assert(htmlV1312.includes('data-spend-account="${escapeHtml(name)}"'),"Spend is not part of accountCardsHtml source-of-truth renderer");
assert(ledgerV1312.includes('bindPersistentSpendActionDelegation'),"Persistent Spend action delegation missing");
assert(ledgerV1312.includes('capabilities:{ accountSpending:true'),"Account spending compatibility capability missing");
assert(htmlV1312.includes('App update incomplete — reload to finish updating'),"Stale-module failure message missing");
assert(htmlV1312.includes('?v=13.0.12')&&swV1312.includes('?v=13.0.12'),"Version-pinned V13.0.12 assets missing");
assert(swV1312.includes('precache().then(() => self.skipWaiting())'),"Service worker atomic activation marker missing");
assert(htmlV1312.includes('availableMoneyAccountCount')&&htmlV1312.includes('min-height:78px !important'),"Phone Budget compaction markers missing");
console.log("V13.0.12 spend reliability and phone Budget compaction checks passed.");
