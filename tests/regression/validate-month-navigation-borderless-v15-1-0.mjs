...
assert.match(runtime, /const VERSION = "2\.0\.1";/);
assert.match(runtime, /const RELEASE_NAME = "Talaan";/);
assert.match(sw, new RegExp(version.cacheVersion));
assert.match(index, new RegExp(version.cacheVersion));
assert.match(index, /sync-runtime-compat\.js\?v=2\.0\.1-talaan1/);
assert.match(index, /Talaan · V2\.0\.1/);
console.log("Talaan V2.0.1 borderless month-navigation regression passed while retaining the compatibility Liquid Glass asset filename.");