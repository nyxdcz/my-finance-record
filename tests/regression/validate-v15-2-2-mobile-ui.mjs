import assert from "node:assert/strict";
import fs from "node:fs";

const read = path => fs.readFileSync(path, "utf8");
const index = read("index.html");
const mobile = read("mobile-v14-0-23.css");
const sw = read("sw.js");
const sync = read("sync-config.js");
const cloud = read("cloud-sync.js");
const updater = read("pwa-update-v15-0-5.js");
const version = JSON.parse(read("version.json"));
const pkg = JSON.parse(read("package.json"));

assert.equal(version.version, "15.2.7");
assert.equal(pkg.version, "15.2.7");
assert.equal(version.schemaVersion, 12);
assert.equal(version.cloudSchemaVersion, 3);
assert.equal(version.cacheVersion, "finance-v15-20260819-application-help-r42");
assert.match(index, /My Finance Records · V15\.2\.7/);
assert.match(index, /mobile-v14-0-23\.css\?v=15\.2\.2-mobile1/);
assert.match(index, /sync-config\.js\?v=15\.2\.7-release1/);
assert.match(index, /const APP_VERSION = "15\.2\.7";/);
assert.match(sw, /const APP_VERSION = "15\.2\.7";/);
assert.match(sw, /finance-v15-20260819-application-help-r42/);
assert.match(sw, /V15\.2\.2 hotfix: Finance workspace sticky row now follows the mobile topbar offset/);
assert.match(sw, /V15\.2\.2 hotfix: compact phone Finance records and icon-only Add account \/ Schedule event actions/);
assert.match(sw, /V15\.2\.2 hotfix: require explicit Cloud Profile selection when an account has multiple finance profiles/);
assert.match(sw, /mobile-v14-0-23\.css\?v=15\.2\.2-mobile1/);
assert.match(sw, /liquid-glass-v15\.css\?v=15\.2\.2-light1/);
assert.match(sw, /sync-config\.js\?v=15\.2\.7-release1/);
assert.match(sync, /const VERSION = "15\.2\.7";/);
assert.match(sync, /const RELEASE_NAME = "Application Help Module Extraction";/);
assert.match(mobile, /V15\.2\.2 · mobile UI \+ UX consistency/);
assert.match(mobile, /body \.workspace-switcher,\s*body \.finance-workspace-marquee-row[\s\S]*var\(--mobile-topbar-offset/);
assert.match(mobile, /\.toast \.toast-dismiss[\s\S]*min-width:\s*44px/);
assert.match(mobile, /\.sidebar \.sidebar-close-button[\s\S]*min-width:\s*44px/);
assert.match(mobile, /\.budget-planner-more-panel[\s\S]*max-width:\s*calc\(100vw - 28px\)/);
assert.match(mobile, /@media \(max-width: 340px\)[\s\S]*data-paid-expense-row/);
assert.match(mobile, /max-height: 520px/);
assert.match(updater, /id = "phoneFinanceCompactV1522"/);
assert.match(updater, /phone-icon-only-action/);
assert.match(updater, /data-pc-add/);
assert.match(updater, /grid-template-areas:"title amount" "due account" "actions actions"/);
assert.match(cloud, /5\*60\*1000/);
console.log("V15.2.7 release preserves V15.2.2 mobile UI/UX and compact Finance source regression.");
