...
const configPosition = index.indexOf("./sync-config.js?v=2.1.0-talaan1");
const runtimePosition = index.indexOf("./sync-runtime-compat.js?v=2.1.0-talaan1");
...
const expectedCache = "finance-v2-20260825-payees-rules-r7";
if (version.version !== "2.1.0") throw new Error(`Unexpected Talaan app version: ${version.version}`);
...
if (!serviceWorker.includes("./sync-config.js?v=2.1.0-talaan1")) throw new Error("Service worker must precache the Talaan sync configuration file.");
if (!serviceWorker.includes("./sync-runtime-compat.js?v=2.1.0-talaan1")) throw new Error("Service worker must precache the Talaan sync runtime compatibility file.");
if (!index.includes(`const APP_CACHE_VERSION = "${expectedCache}";`)) throw new Error("index.html cache identity is not synchronized.");
if (!index.includes("Talaan · V2.1.0")) throw new Error("Prepared index title is not Talaan V2.1.0.");
...
console.log("Sync configuration separation validated under Talaan V2.1.0.");