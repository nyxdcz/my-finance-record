...
assert.match(css,/html\[data-theme="light"\] #availableMoneySection \.account-card-label span \{[\s\S]*color:var\(--text\) !important;/);
assert.match(css,/html\[data-theme="light"\] #availableMoneySection \.account-card-label small \{[\s\S]*color:var\(--muted\) !important;/);
...
console.log("V15.2.18 light/dark palette regression passed with readable Available money account labels in light mode.");
