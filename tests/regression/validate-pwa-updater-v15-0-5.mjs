...
assert.match(index, /<strong>Talaan<\/strong>/, "Prepared HTML must own the Talaan sidebar brand");
...
assert.match(updater, /const UI_HOTFIX_REFRESH_KEY = "finance-ui-hotfix-v2-0-1-talaan1";/);
...
assert.match(index, /phone-finance-compat\.js\?v=2\.0\.1-talaan1/, "Index must load the Talaan Phone Finance compatibility entry");
assert.match(worker, /phone-finance-compat\.js\?v=2\.0\.1-talaan1/, "Service worker must precache the Talaan Phone Finance compatibility entry");
...
assert.match(worker, /const APP_VERSION = "2\.0\.1";/);
...
assert.match(worker, /pwa-update-v15-0-5\.js\?v=2\.0\.1-talaan1/);
...
console.log("Talaan V2.0.1 PWA updater regression passed with current cache delivery and compatibility ownership preserved.");
